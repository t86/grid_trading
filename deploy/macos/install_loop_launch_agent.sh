...
LABEL="${LABEL:-com.tl.grid-optimizer.loop}"
ROOT_DIR="$(cd "$(dirname "$0")/../.." && pwd)"
AGENT_DIR="$HOME/Library/LaunchAgents"
PLIST_PATH="$AGENT_DIR/${LABEL}.plist"
LOG_DIR="$ROOT_DIR/output/launchd"
LAUNCHD_LOG_DIR="$HOME/Library/Logs/grid-optimizer-loop"
RUNTIME_DIR="$HOME/Library/Application Support/grid-optimizer-loop"
STDOUT_LOG="$LAUNCHD_LOG_DIR/stdout.log"
STDERR_LOG="$LAUNCHD_LOG_DIR/stderr.log"
START_SCRIPT="$ROOT_DIR/deploy/macos/start_grid_loop.sh"
WRAPPER_SCRIPT="$RUNTIME_DIR/start_grid_loop.sh"
