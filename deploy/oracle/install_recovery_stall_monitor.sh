...
TIMER_UNIT_NAME="${TIMER_UNIT_NAME:-grid-recovery-stall-monitor}"
SYMBOLS="${SYMBOLS:-BILLUSDT}"
OUTPUT_DIR="${OUTPUT_DIR:-${APP_DIR}/output}"
STATE_PATH="${STATE_PATH:-${APP_DIR}/output/recovery_stall_monitor_state.json}"
ALERT_CONFIG_PATH="${ALERT_CONFIG_PATH:-${APP_DIR}/output/recovery_stall_alert_config.json}"
...
[Unit]
Description=Grid recovery stall monitor
...
ExecStart=${PYTHON_BIN} -m grid_optimizer.recovery_stall_monitor --symbols ${SYMBOLS} --output-dir ${OUTPUT_DIR} --state-path ${STATE_PATH} --alert-config-path ${ALERT_CONFIG_PATH} --threshold-seconds ${THRESHOLD_SECONDS}
EOF

sudo tee "$TIMER_FILE" >/dev/null <<EOF
[Unit]
Description=Run Grid recovery stall monitor
