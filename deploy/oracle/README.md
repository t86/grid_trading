...
`<control>.last_valid` 快照，保证主 control 文件结构损坏时只能先恢复上一个已验证状态、不会失去恢复路径。自定义 runner/watchdog 命名时必须通过 `RUNNER_WATCHDOG_TIMER_TEMPLATE` 提供对应 timer 模板。运行中的正目标窗口还要求 `competition_target_gate` 的 `*_target_gate_heartbeat.json` 与当前 `run_contract_id` 匹配且新鲜；安装器默认以 600 秒为上限，可通过 `TARGET_GATE_MAX_HEARTBEAT_AGE_SECONDS` 收紧。该宽限结束后，watchdog 还必须能验证当前 run contract 的 `*_terminal_intent.json`；仅有新鲜 heartbeat 不能掩盖 deadline 后没有终止 owner 的故障。最后还必须发现
