...
- `spot_competition_synthetic_neutral_grid` 已内置该路径：仅消费本 runner 在功能启用时间之后已登记的 maker BUY 成交；要求最新买一不低于成交价、买一可见数量覆盖待卖数量并满足交易所最小值。每个成交 ID 只提交一次，冻结 maker 成交不会进入该路径。
- 若实际 base 仍低于 `neutral_base_qty`，底舱回补优先：任何回补 BUY 都不得触发同价 TAKE；只有回到中性底舱容差内后的下一笔 maker BUY 才重新启用该路径。
