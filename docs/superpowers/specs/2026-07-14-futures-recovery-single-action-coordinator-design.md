...
Web 同样不得把“控制文件存在但无法解析”降级为新建空配置：启动、停止、保存、快速操作和触发器路径在外层返回可见 `runner_control_unreadable`，底层控制保存也拒绝覆盖原字节。legacy guard 的锁内控制更新同样拒绝不可读或非对象文件，不能用先前内存快照回写、重启运行器或伪造新的所有权。只有文件确实不存在时，未注册兼容路径才可创建首份控制；不可读文件必须保留给受管修复/人工迁移，不能作为绕过单一所有权的入口。
