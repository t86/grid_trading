...
上一轮已列为受管的 symbol，即使控制文档随后 JSON 损坏、只剩一个恢复槽，或无法读取，也必须继续按受管对象失败关闭：主循环以持久 heartbeat 中的已注册列表和残留恢复槽作为“不可回退”证据，路由协调器返回可告警的 `registered_recovery_blocked`。只有当前控制文档可读、明确不含恢复槽，且上一轮未记录该 symbol 为受管时，才允许走未注册兼容路径；控制损坏绝不能重新授权 legacy 守卫的撤单、停机或重启。
