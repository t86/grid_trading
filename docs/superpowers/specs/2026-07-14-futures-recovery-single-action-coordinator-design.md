...
上一轮已列为受管的 symbol，即使控制文档随后 JSON 损坏、只剩一个恢复槽，或无法读取，也必须继续按受管对象失败关闭：主循环以持久 heartbeat 中的已注册列表和残留恢复槽作为“不可回退”证据，路由协调器返回可告警的 `registered_recovery_blocked`。独立只读 watchdog 看到控制文件存在但不可读时也必须将该 symbol 标为 `recovery_control_unreadable` 并按阈值告警，不能把它误报为未注册健康；它只报告，不执行恢复动作。协调器写入控制时同时原子保留最后一份有效副本；只有主文件结构性不可读、该副本能独立验证完整双恢复槽、symbol、代际和扁平 managed profile 时，协调器本轮唯一动作才可为 `registered_recovery_control_snapshot_restored`，不重启、不撤单、不下单，下一轮重新仲裁。副本回写失败也必须返回 critical blocked，不能抛出未处理异常或报告恢复成功。主文件可读但 slots 分歧、摘要/扁平字段冲突，或副本任何一项无效时必须保持 critical blocked，绝不以旧副本覆盖可能更新的控制。只有当前控制文档可读、明确不含恢复槽，且上一轮未记录该 symbol 为受管时，才允许走未注册兼容路径；控制损坏绝不能重新授权 legacy 守卫的撤单、停机或重启。

Web 同样不得把“控制文件存在但无法解析”降级为新建空配置：启动、停止、保存、快速操作和触发器路径在外层返回可见 `runner_control_unreadable`，底层控制保存也拒绝覆盖原字节。legacy guard 的锁内控制更新同样拒绝不可读或非对象文件，不能用先前内存快照回写、重启运行器或伪造新的所有权。health monitor 在未知所有权下只观察，不得重启、死锁平仓或修改偏移；target gate 的手动冻结 TP 也必须返回 `BLOCKED_UNREADABLE_RECOVERY_CONTROL`，不得提交订单；liveness terminal guard 也不得转调旧 watchdog。只有文件确实不存在时，未注册兼容路径才可创建首份控制；不可读文件必须保留给受管修复/人工迁移，不能作为绕过单一所有权的入口。
...
普通净敞口上限统一使用 `ordinary_long = exchange_long - frozen_long`、`ordinary_short = exchange_short - frozen_short`、`ordinary_net = ordinary_long - ordinary_short`，并先验证冻结分侧数量不大于交易所对应分侧仓位；不允许用 `max(..., 0)` 静默夹零。交易所账户级 `LONG - SHORT` 只用于观测和冻结账本对账；冻结仓位只进入冻结账本、冻结分侧/总量上限及冻结修复，不能进入 `max_actual_net_notional`、普通多空软阈值或硬上限。Hedge 模式的普通仓位中，`BUY/LONG entry` 与 `BUY/SHORT reduce` 都增加 `ordinary_net`，`SELL/LONG reduce` 与 `SELL/SHORT entry` 都减少 `ordinary_net`；任何角色名、`allow_loss` 或局部 guard 都不得改变这个方向语义。所有 entry pause、inventory unlock、亏损许可、恢复 profile 和普通仓位上限处理完后，提交器执行一次最终普通净敞口裁决，且不允许相反方向普通委托互相抵扣：若普通现有委托的最坏单边成交会越界，本轮唯一动作是撤销精确归属的普通增险方向委托；确认撤单后的下一轮，才可从已经通过上游门禁的候选中保留最多一个 `LIMIT + GTX` 普通降险委托；无安全候选时明确 `HOLD`，不得重新生成被波动暂停删除的 entry，也不得自行开启亏损权限。

普通仓位只能在本轮账户快照和冻结账本快照都读取完成后计算一次，随后作为不可变输入传给普通控制链；后续分支不得再从交易所总仓、策略内部仓位或旧计划字段各自重算。首次计算前必须先用本轮权威成交事实依次结算冻结 manual、per-lot binding 和 pair release，避免“交易所已减仓、冻结账本尚未减仓”被永久误判为冻结数量大于交易所仓位；三条同步每轮各执行一次，之后才固化普通仓位快照。`volatility_entry_pause` 的库存分量、elastic volume、regime entry budget、BQ inventory bias、普通 projected entry cap、恢复方向、恢复完成条件及动作排序都只读取这组普通仓位。`frozen_v2` 只能保留冻结账本观测或进入冻结所有者自己的动作，不能缩放、暂停、放宽或反转普通 maker entry。冻结 ledger 的 summary-only 或 lots-only 兼容既有数据；二者同时存在时必须分侧相等，坏 lot、负数、布尔值、非有限数或冻结数量大于交易所对应分侧仓位均失败关闭并进入可见账本修复，不能夹零、猜测或退回交易所总仓继续判断。

定期对账的仓位真相始终来自 REST；用户数据流只用于低延迟观测、订单缓存和健康告警，不能把漏掉的账户更新解释为一致。若冻结账本本身仍有效、但与当前交易所分侧仓位暂时不一致，不能因此无限暂停普通刷量：普通 `BUY/LONG` 或 `SELL/SHORT` entry 只在其对应的 `exchange - frozen` 普通侧已超过本轮计划时拒绝；该侧没有普通超额时可继续提交。普通减仓仍受可用普通数量的严格约束，冻结修复仍由冻结所有者单独处理；任何无效账本或冻结数量超过交易所仓位继续失败关闭。

定期对账发现的交易所实际净仓与本轮计划净仓不一致，必须按连续确认计数进入协调器安全观测；单次差异只记录，第二次连续差异才触发 `RUNNER_RECOVER` 候选，下一次 REST 一致立即把计数清零。运行器不得据此直接撤单、停止或重启；对已注册 symbol 只发布绝对 TTL 信号，后续动作仍由协调器的单动作优先级选择。

这里的“完全隔离”同时覆盖数量、成本和订单三类输入。普通多空软阈值、硬上限、净敞口上限、单边亏损放开条件、普通减仓可用量和恢复完成条件只能使用 `ordinary_long/ordinary_short`；普通成本门禁只有在普通 BQ ledger 与本轮权威普通仓位一致时才可使用旧 lot 成本，否则回退到本轮 `exchange - frozen` 的普通数量和对应交易所成本，不能把冻结 lot 的数量或成本重新混入。普通开放委托数、缺失委托数、陈旧委托数、最大委托预算和 plan-to-submit 漂移也只统计 `normal_bq`；冻结订单独立成交、撤销或重报价不得触发普通恢复，也不得阻断普通提交。

该最终裁决只收窄普通候选，不取得冻结账本或临时亏损租约的所有权，普通 runtime signal、下单 fence 和撤单 fence 均不得删除、选择或撤销冻结 lane 的动作。`TEMPORARY_LOSS_RELIEF` 仍必须绑定当前代际、决策、方向、订单角色和绝对 TTL，到期或回执完成后自动回收；`volatility_entry_pause` 默认开启；任何恢复订单继续保持 maker-only。运行时 position-cap 信号必须由 `ordinary_net` 生成有符号方向：普通净多只允许 SELL 降险，并把已授权的普通风险 BUY 撤单与旧 SELL 降险单刷新撤单都交给最终裁决二选一，普通净空完全对称；有待撤订单时不得在同轮下新单。即使交易所总净仓与普通净仓符号相反，也只能按普通净仓方向恢复。
...
启用普通净敞口上限时，提交前必须强制读取该 symbol 的完整、非缓存交易所开放委托，再按订单账本分类后做最坏单边投影，不能只依赖可能仅含策略前缀订单的 user-data-stream 本地缓存。明确属于 `frozen_bq` 的仓位、开放委托、候选委托和撤单完全退出普通裁决，冻结 pair 两腿不得被拆开，冻结 manual reduce 也不得冒充普通降险动作；冻结专属动作由冻结所有者独立处理，并作为该 symbol 当前循环的一个逻辑动作。交易所开放委托不能凭 role、book 或 client ID 前缀自称 frozen。冻结归属与冻结动作授权必须分离：当前 state 中精确匹配 order ID/client ID 的 durable `frozen_bq` order ref，或 pair submission manifest 中的精确腿身份，单独决定该订单属于冻结通道，因此即使 directive 丢失、损坏或过期，也绝不能降级为普通订单并被普通撤单/终止排空删除；directive 的 request ID、role、side、剩余数量和绝对有效期只决定是否允许继续执行冻结 DELETE/POST。没有 durable ref/manifest 的相似前缀订单一律按 unknown 参与保守投影且不取得冻结所有权或撤单权。

symbol 级选择顺序固定为：已成交缺腿且有显式 `repair_side` 的冻结 pair repair；普通硬上限安全动作或失败关闭 `HOLD`；当前已激活且有 deadline/lease 的普通恢复（包括 `TEMPORARY_LOSS_RELIEF`）；普通冻结请求；普通正常刷量。普通冻结请求按持久化 `last_selected_at` 做 oldest-first 轮转，每次实际选择都写入回执；不能每轮固定选择候选列表第一项。冻结 pair 虽然包含两次交易所请求，但预算不足以容纳两腿时必须整组延后，不能提交单腿；只有 directive 的 `filled_long_qty != filled_short_qty`、由差值推导的唯一缺腿方向与显式 `frozen_inventory_pair_repair_side` 完全一致时，单腿才是合法 repair。所有冻结 manual、per-lot 和 pair 请求同样严格使用 `LIMIT + GTX + post_only`；IOC/aggressive 即使携带 request ID 也没有旁路资格。

`frozen_inventory_request_id` 只是索引，不是授权。计划 gate、动作 gate、每次 DELETE 和每次 POST 前都必须重新读取当前 state，精确校验 directive 类型、方向、request ID、`requested/consumed`、绝对 expiry、剩余数量和冻结 ledger；recovered manual-limit 只在对应 side 的 isolated qty 为正且 ledger request ID 精确匹配时兼容。普通 recovery profile、runtime signal、下单 fence 和撤单 fence 只旁路这种已重新验证的冻结动作。未知或人工委托仍参与保守风险证明，但不因此取得撤单权限；若未知增险委托使普通上限不可证明，本轮明确 `HOLD` 并报告 `risk_cancel_not_authorized`。所有上限、估值价和当前普通净数量都必须是有限数；冻结账本数量超过交易所对应 side 时进入冻结账本修复/失败关闭，不得通过 `max(..., 0)` 静默解锁或截断后回写账本。`NaN`、正负无穷或无效配置在启动配置层直接拒绝，提交层再次失败关闭。

runner state 的所有读改写同样按 symbol 单一所有权执行。运行器从本轮首次读取 state 起，到计划、提交、冻结回执、对账和本轮最终状态原子替换完成为止，持有同一份可重入的跨进程 state lock；Web 冻结指令入口也必须先取得该锁，随后重新读取最新 state，再原子替换。只给 `write_json` 加锁或只使用临时文件 rename 不能防止旧快照覆盖新 directive/manifest/receipt，因此禁止。涉及 state 与 recovery control 两种资源时统一按 `state -> control` 获取，锁内重新检查 coordinator ownership；禁止任何 `control -> state` 路径形成 ABBA。控制文件不存在才是未注册兼容的空控制；文件存在但 JSON 损坏、读失败或不是对象时，任何旧执行器（包括 `competition_state_realign`）都必须返回可见的 `BLOCKED_UNREADABLE_RECOVERY_CONTROL`，不得停止服务、撤销委托、重写账本、归档计划或覆盖原控制字节。Web 发现 state path 漂移时只做一次有界重试，仍漂移则返回可重试错误；旧 guard 的损坏状态修复、冻结账本恢复和 `competition_state_realign` 也必须使用同一 state transaction。状态重对齐在锁内再次检查 coordinator ownership，只有未被接管且控制所有权可确定时才可停止运行器、撤销普通委托、重写普通账本和恢复它自己停止的服务；它使用与 runner/Web 相同的 durable order-ref/Pair manifest 精确归属，任何冻结委托都不得被旧 `gx-` 前缀批量撤单。锁文件只要求所有进程可读，并在只读文件描述符上取得独占建议锁，以兼容 root guard 首次创建、非特权 runner/Web 随后接管；锁等待超时必须返回可见错误并保持原文件不变，不能绕过锁补写。崩溃由操作系统释放文件锁，下一轮从 durable manifest/receipt 继续。

终止排空只拥有普通通道。terminal owner 可以固化本次运行契约和普通订单清单，但不能把启动时的冻结数量当作不可变化的 floor；每轮都用当前权威冻结账本从当前交易所分侧仓位中扣除后，判断普通残仓、普通开放委托和普通排空完成。冻结 manual/pair 委托在终止期间独立成交或更新账本，不构成运行契约篡改，不得产生 `frozen_ledger_mismatch`、`owned_inventory_exceeds_exchange`，也不得被 legacy stop/quick-flatten 或 terminal cancel 撤销。
...
交接检查还必须验证三项新增活性事实：控制主文件之外存在可独立校验的最后有效快照，target-gate 调度器真实处于活跃状态，且该 symbol 的 runner watchdog timer 真实处于活跃状态（默认 `grid-loop-watchdog@<SYMBOL>.timer`，自定义服务模板必须显式提供对应 timer 模板）。target-gate 必须带 `--enforce`；dry-run 既不能作为交接前的有效 scheduler，也不能写入运行 heartbeat。前者只在主文件结构性不可读时用于单轮无交易副作用的恢复；后两者分别保证截止时会形成唯一终止 intent、runner 停止后会重新唤起同一冻结 owner，并不授权脚本自行停进程、撤单或改变仓位。target-gate 在成功校验当前冻结运行契约后还必须写入带 `run_contract_id` 的只读 heartbeat；独立协调器 watchdog 仅在正目标的活动窗口（及一个有限收尾宽限）要求它新鲜，缺失、陈旧、symbol/契约不匹配都升级为严重可告警状态，不能静默等到错过截止。收尾宽限结束后，heartbeat 不再可以单独证明成功：必须存在通过完整校验、且 `run_contract_id` 匹配当前冻结契约的 terminal intent（pending 或完成态均可）；缺失、不可读、伪造或旧契约 intent 都升级为严重可告警状态。三项交接事实、运行中的 heartbeat 或 deadline 后的唯一终止 owner 任一不成立都拒绝/阻断健康判定，避免“恢复器已接管但没有截止处理者”的静默无量状态。
...
77. 未启用临时放亏窗口预算时，`futures_run_contract_snapshot_v3` 仍是规范运行快照；启用后使用 `futures_run_contract_snapshot_v4` 和 `futures-run-contract-v4-<digest>`。运行器、目标闸门和 watchdog 必须复算并验证同一摘要。
