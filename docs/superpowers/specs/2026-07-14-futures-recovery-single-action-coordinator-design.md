...
对已显式注册到恢复协调器的合约策略，启动本身一律视为有界运行：`run_end_time` 与 `max_cumulative_notional` 至少必须有一项。缺少二者时 saved-runner 必须在创建进程、写 PID、修改控制文档或触碰订单前拒绝启动；这不是暂停，也不是允许无限追逐的默认模式。未注册的兼容/诊断策略仍可沿用旧启动路径，但不能借此取得协调器所有权。
