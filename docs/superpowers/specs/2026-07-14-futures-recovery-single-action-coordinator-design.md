...
交接检查还必须验证两项新增活性事实：控制主文件之外存在可独立校验的最后有效快照，且 target-gate 调度器真实处于活跃状态。前者只在主文件结构性不可读时用于单轮无交易副作用的恢复；后者只保证截止时会形成唯一终止 intent，并不授权脚本自行停进程、撤单或改变仓位。两项任一不成立都拒绝交接，避免“恢复器已接管但没有截止处理者”的静默无量状态。
