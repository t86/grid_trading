...
状态：用户已确认设计；第一阶段失败测试、通用决策核心、持久化适配器和只读影子适配器已实现、完成回归并提交到隔离分支。生产所有权切换保持关闭。
...
对最新 `main` 的复核还确认了几条会直接导致长期无量的具体路径：动作验证失败可以停在没有退出边的 `failed_hold`；临时恢复开始时间会被后续分支重置而缺少不可续期硬截止；交易所成交查询失败会在回收逻辑前提前跳过本轮；控制写入和重启完成后才保存守卫状态，崩溃会丢失原基线/租约；部分 runtime stop reason 会被非活跃恢复永久跳过；单次交易所空订单观测即可触发重启；Web、目标闸门、滚动、磨损守卫和 systemd 仍保留绕过单一所有权的执行路径。这些不是阈值调优问题，而是缺少统一决策、方向化诊断、持久化阶段和恢复活性契约。
...
本分支在设计确认后进入第一阶段实现：新增通用决策核心、持久化适配器、只读影子适配器和失败/表征测试，并对旧守卫中“绝对恢复超时被验证等待覆盖”及“临时放亏回收值不确定”做最小修正；不修改运行配置、部署状态或任何生产调用入口。新增模块默认没有生产调用方，旧生产刷量路径仍保持原所有权。合并前必须基于最新 `origin/main` 重放回归；不得用本分支较早的代码覆盖后续生产修复。

当前 `main` 已包含的普通 BQ 入场受阻时定向 reduce-only、双侧 entry 受阻时恢复减仓单、ARX 低速时恢复双边近价 maker/临时容量、partial maker book drift 和“近期 submit 只抑制重启但不抑制独立流量恢复”等修复，全部作为迁移兼容门禁：先将其现有测试与真实故障轨迹纳入 characterization suite，再映射到 `FlowBlockerAssessment` 和标准动作；影子决策未逐项等价前，不删除、改序或部署替换旧路径。
...
1. `TERMINAL_STOP`（终止停机）
   - 只允许用于：当前竞赛窗口目标已确认完成；已证明无法从已注册运行时配置重建的受管文档损坏；或已注册策略明确定义的终止型业务条件。
...
7. `MAKER_FLOW_RECOVER`（挂单方流量恢复）
   - 成交量进度、Web 成交量触发意图、报价距离、容量、单边挂单长期无进展或挂单方流量恢复。
...
`INVENTORY_RECOVER`、`TEMPORARY_LOSS_RELIEF`、`MAKER_FLOW_RECOVER` 或 `BASELINE_TUNE` 已正确激活，但在各自动作定义的有效性期限内没有产生规定方向的新受管 GTX 委托、订单进展或真实成交时，动作记录 blocker、方向、证据指纹和 `episode_exhausted`。若该决策可能已有 ActionLease 订单，先原子设置 `allow_loss=false`、建立/合并 `cleanup_obligation` 并进入 `Cleaning`；确认该 epoch 无未终结订单后才通过自己的 `EXIT` 还原完整基线。没有此类订单时可直接 `EXIT`。下一轮才按全局优先级选择另一个合格动作。不能在同一轮追加第二个动作，也不能在证据未变化时立即重复同一无效动作。
...
“按 `allow_loss=false` 处理”只阻止新规划和新提交，不足以完成回收。协调器必须持久化并推进 `cleanup_obligation`，精确撤销该 `action_lease_epoch` 的未终结 GTX 订单，并确认撤单竞态成交已经进入 `userTrades`/库存账本。交易所 API 暂时失败时，旧租约订单清理保持待处理并受栅栏重试，但新亏损放宽订单始终禁止；API 恢复后自动完成，不依赖外部清理事件。硬截止时间不因撤单等待而延长，安全/终止抢占和每日滚动也不能删除或绕过 obligation。
...
- 成交量触发适配器只能映射到 `MAKER_FLOW_RECOVER`，波动率触发适配器只能映射到 `SAFETY_CONVERGE`/`SafetyEnvelope` 更新；二者自身绝不能产生 `TERMINAL_STOP`。当前窗口目标完成只由目标闸门的白名单证据产生，其他真正终止型业务条件必须来自独立注册且表驱动校验的终止适配器。
...
- 只有没有 safety/action lease、未终结动作订单、`cleanup_obligation`、待处理激活/执行阶段且动作明确可重建时，才在一次原子提交中更新规范基线、递增代际并物化扁平化 `control`。
- 若仍有临时授权或订单，滚动请求先返回 `deferred`，当前动作自行提交 `allow_loss=false`、完成 `EXIT/managed_gtx_cancel` 和清单对账；滚动不能把“清除覆盖层”冒充“回收订单”。清理确认后的新尝试才可提交基线。
- 不再单独清除会影响正确性的保护状态。
...
- 对受管 symbol，目标闸门旧的 `systemctl stop` + MARKET 平仓路径必须禁用并改为提交 `TERMINAL_STOP`；滚动和磨损守卫必须委托协调器；Web 触发器只能提交意图；runner unit 必须在协调器监督与心跳门禁生效后切为 `Restart=no`。
...
本分支当前只完成步骤 2、4、5、6 的核心基础部分：动作注册表、每交易对单动作状态机、完整配置物化、绝对截止时间、临时放亏租约与自动回收、方向公平游标、跨重启 JSON/CAS 状态和旧评估结果的只读影子映射。它尚未完成真实交易所快照、订单意图 journal、双查询清理证明、类型化执行器/回执接线、严重告警出口、旧写入方盘点和生产所有权切换。因此本次提交只能作为默认关闭的影子/基础设施，不能注册或启用任何生产交易对；特别是 `liveness_status=blocked` 在真实严重告警与后继诊断闭环完成前属于切换硬阻塞，不能被当作可长期接受的暂停状态。
...
68. 当前 main 的 BQ 定向减仓/双侧 entry 阻塞与 ARX 低速双边 maker、容量、partial-drift 修复先作为表征门禁保留；本规范分支不改生产代码，未来影子决策未等价前不得删除、覆盖或部署替换。
