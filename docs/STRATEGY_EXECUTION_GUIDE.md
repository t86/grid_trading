...
- 当前 `spot_freeze` 的现货执行腿是 `MARKET`，不是 maker。代码默认不允许自动市价冻结；只有显式设置 `spot_freeze_market_execution_enabled=true` 才会在 apply 模式下执行，否则 eligible 时会报 `market_execution_disabled` 并保持诊断状态。目标是尽量 make 时，正式启用自动冻结前必须先完成 maker 化或保持 dry-run，只把 APP 损耗 guard 用作减偏离方向的 maker 过滤。
