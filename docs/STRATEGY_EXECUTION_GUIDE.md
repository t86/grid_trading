...
- 交易赛现货恢复 APP loss guard/recovery 时还必须启用有效的 `spot_freeze_enabled=true` 和 `spot_freeze_maker_execution_enabled=true`；`spot_freeze_deviation_notional` 不能大到触发 `deviation_threshold_disables`，否则冻结仓位只是名义开启，不能恢复刷量。
