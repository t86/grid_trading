...
- 如果 `spot_freeze_skip_reason=total_cap_reached`、`short_hedge_capacity_exhausted` 或 `insufficient_short_hedge_to_freeze_long`，runner 必须 fail-closed：清空本轮订单、写入 `spot_freeze_runtime_blocked=true` 和对应 `risk_state`，不能继续裸跑普通 grid 单。
