...
class BestQuoteMakerVolumeTests(unittest.TestCase):
    ...
    def test_four_leg_profit_reduce_uses_unreleased_entry_lot_and_retains_inventory_floor(self) -> None:
        plan = build_best_quote_maker_volume_plan(
            ...
            inputs=_inputs(
                ...
                step_size=0.001,
                position_side_mode="hedge",
                current_long_qty=3.95,
                current_short_qty=3.95,
                current_long_avg_price=101.0,
                current_short_avg_price=99.0,
                current_long_lots=[
                    {
        ...
        self.assertLessEqual(short_reduce["notional"], 195.0 + 1e-12)
        self.assertLessEqual(short_reduce["price"], 98.3)
        report = plan["metrics"]["four_leg_cycle"]["roles"]["short_profit_reduce"]
        self.assertEqual(report["cost_source"], "ordinary_entry_lot")
    ...
    def test_four_leg_profit_reduce_does_not_repeat_without_an_unreleased_entry_lot(self) -> None:
        ...
        self.assertNotIn("best_quote_reduce_long", roles)
        self.assertNotIn("best_quote_reduce_short", roles)
        report = plan["metrics"]["four_leg_cycle"]["roles"]
    ...
    def test_four_leg_profit_reduce_stops_at_retained_inventory_floor(self) -> None:
        plan = build_best_quote_maker_volume_plan(
            ...
            inputs=_inputs(
                ...
                current_short_avg_price=99.0,
                current_long_lots=[
                    {"qty": 2.0, "price": 101.0, "role": "best_quote_entry_long"}
                ],
                current_short_lots=[
                    {"qty": 2.0, "price": 99.0, "role": "best_quote_entry_short"}
                ],
            ),
        )
        ...
        self.assertNotIn("best_quote_reduce_short", roles)
        report = plan["metrics"]["four_leg_cycle"]["roles"]
        self.assertEqual(report["long_profit_reduce"]["reason"], "retained_inventory_floor")
        self.assertEqual(report["short_profit_reduce"]["reason"], "retained_inventory_floor")

    def test_four_leg_lot_guard_does_not_remove_existing_loss_control(self) -> None:
        plan = build_best_quote_maker_volume_plan(
            config=BestQuoteMakerVolumeConfig(
                enabled=True,
                four_leg_cycle_enabled=True,
        ...
        self.assertIn("best_quote_reduce_long", roles)
        self.assertIn("best_quote_reduce_short", roles)
        report = plan["metrics"]["four_leg_cycle"]["roles"]
        self.assertEqual(report["long_profit_reduce"]["reason"], "loss_control_active")
        self.assertEqual(report["short_profit_reduce"]["reason"], "loss_control_active")

    def test_four_leg_cycle_reports_missing_ordinary_cost(self) -> None:
        plan = build_best_quote_maker_volume_plan(
            config=BestQuoteMakerVolumeConfig(
                enabled=True,
                four_leg_cycle_enabled=True,
    ...
    def test_normal_entries_stop_at_soft_inventory_headroom(self) -> None:
        plan = build_best_quote_maker_volume_plan(
            config=BestQuoteMakerVolumeConfig(
                enabled=True,
                max_entry_orders_per_side=2,
            ...
            inputs=_inputs(
                ...
                ask_price=1.01,
                mid_price=1.0,
                current_long_qty=850.0,
                current_short_qty=870.0,
                current_net_qty=0.0,
        ...
        long_entry_notional = sum(
            order["notional"]
            for order in plan["buy_orders"]
            if order["role"] == "best_quote_entry_long"
        )
        short_entry_notional = sum(
            order["notional"]
            for order in plan["sell_orders"]
            if order["role"] == "best_quote_entry_short"
        )
    ...
    def test_inventory_bias_stops_heavy_side_and_caps_light_side_at_soft_headroom(self) -> None:
        plan = build_best_quote_maker_volume_plan(
            config=BestQuoteMakerVolumeConfig(
                ...
                inventory_bias_reduce_share=0.0,
    ...
    def test_four_leg_cycle_keeps_smaller_heavy_side_entry_under_bias(self) -> None:
        plan = build_best_quote_maker_volume_plan(
            config=BestQuoteMakerVolumeConfig(
                enabled=True,
                four_leg_cycle_enabled=True,
        ...
        long_entry_notional = sum(order["notional"] for order in long_entries)
        short_entry_notional = sum(order["notional"] for order in short_entries)
        self.assertLess(long_entry_notional, short_entry_notional)
        self.assertLessEqual(long_entry_notional, 45.0 + 1e-6)
        self.assertEqual(
            plan["metrics"]["four_leg_cycle"]["roles"]["long_entry"]["status"],
            "planned",
        )

    def test_four_leg_cycle_reports_heavy_side_headroom_below_minimum(self) -> None:
        plan = build_best_quote_maker_volume_plan(
            config=BestQuoteMakerVolumeConfig(
                enabled=True,
                four_leg_cycle_enabled=True,
        ...
        self.assertEqual(
            plan["metrics"]["four_leg_cycle"]["roles"]["long_entry"],
            {
                "status": "blocked",
    ...
    def test_four_leg_cycle_reports_normal_side_headroom_below_minimum(self) -> None:
        plan = build_best_quote_maker_volume_plan(
            config=BestQuoteMakerVolumeConfig(
                enabled=True,
                four_leg_cycle_enabled=True,
                ...
                inventory_soft_ratio=0.9,
                inventory_bias_enabled=True,
                inventory_bias_min_notional_gap=100.0,
            ),
            inputs=_inputs(
                bid_price=1.0,
                ask_price=1.01,
                mid_price=1.0,
                current_long_qty=894.0,
                current_short_qty=800.0,
                current_long_avg_price=1.03,
                current_short_avg_price=0.98,
                cycle_budget_notional=240.0,
