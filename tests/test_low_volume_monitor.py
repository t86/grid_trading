...
class LowVolumeMonitorTests(unittest.TestCase):
    def test_summarize_recent_volume_uses_trade_time_window(self) -> None:
        now = datetime(2026, 5, 18, 0, 10, tzinfo=timezone.utc)
        rows = [
            {"time": int((now - timedelta(minutes=11)).timestamp() * 1000), "quoteQty": 900},
            {"time": int((now - timedelta(minutes=9)).timestamp() * 1000), "quoteQty": 400},
            {"time": int((now - timedelta(minutes=1)).timestamp() * 1000), "quoteQty": 250},
        ]

        summary = summarize_recent_volume(rows=rows, now=now, window_seconds=600)

        self.assertEqual(summary["trade_count"], 2)
        self.assertAlmostEqual(summary["gross_notional"], 650.0)

    def test_low_volume_alerts_once_and_resets_after_recovery(self) -> None:
        now = datetime(2026, 5, 18, 0, 10, tzinfo=timezone.utc)
        with TemporaryDirectory() as tmpdir:
            ...
            trade_path.write_text(
                "\n".join(
                    [
                        '{"time": %d, "quoteQty": 400}' % int((now - timedelta(minutes=9)).timestamp() * 1000),
