...
class WebSecurityTests(unittest.TestCase):
    ...
    def test_running_status_page_contains_frozen_inventory_ledger_controls(self) -> None:
        ...
        self.assertIn("已由 post-only 跟价单接管的冻结仓位不再参与对等释放", page)
        self.assertIn("清理冻结多仓数量", page)
        self.assertIn("清理冻结多仓", page)
        self.assertIn("清理会下 IOC reduce-only", page)
    ...
    def test_resolve_runner_start_config_starts_bch_altcoins_probe_with_maker_safety_guards(
        ...
        self.assertIn("--no-best-quote-maker-volume-four-leg-cycle-enabled", command)
        ...
        self.assertFalse(runner_args.best_quote_maker_volume_four_leg_cycle_enabled)
