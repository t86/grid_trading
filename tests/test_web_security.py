...
class WebSecurityTests(unittest.TestCase):
    ...
    def test_runner_preset_payload_applies_xaut_short_adaptive_profile(self) -> None:
        ...
        self.assertFalse(payload["autotune_symbol_enabled"])
        self.assertEqual(payload["step_price"], 7.6)
        self.assertEqual(payload["base_position_notional"], 260.0)
        self.assertEqual(payload["pause_short_position_notional"], 600.0)
        self.assertEqual(payload["max_short_position_notional"], 660.0)
        self.assertEqual(payload["inventory_tier_start_notional"], 360.0)
        self.assertEqual(payload["inventory_tier_end_notional"], 500.0)
        self.assertEqual(payload["inventory_tier_buy_levels"], 14)
        self.assertEqual(payload["inventory_tier_sell_levels"], 2)
        self.assertEqual(payload["inventory_tier_per_order_notional"], 65.0)
        self.assertEqual(payload["inventory_tier_base_position_notional"], 120.0)
