...
class SubmitPlanTests(unittest.TestCase):
    ...
    def test_directional_recovery_replaces_authorized_stale_balancing_entry(self) -> None:
        existing_entry = {
            "orderId": 83,
            "side": "SELL",
            "positionSide": "SHORT",
        ...
        protected_cancel = dict(existing_entry)
        closer_entry = {
            "side": "SELL",
            "position_side": "SHORT",
            "price": 201.0,
        ...
        guarded = apply_actual_net_exposure_decision_to_actions(
            actions={
                "place_orders": [closer_entry],
                "cancel_orders": [],
                "same_side_spacing_guard": {
                    "protected_cancel_orders": [protected_cancel],
                },
            },
            current_actual_net_qty=0.50,
            valuation_price=200.0,
        ...
        self.assertEqual(decision["stale_balancing_entry_price"], 203.0)
        self.assertEqual(decision["replacement_balancing_entry_price"], 201.0)
        self.assertEqual(guarded["cancel_orders"], [protected_cancel])
        self.assertEqual(guarded["place_orders"], [])

    def test_directional_recovery_keeps_entry_when_replacement_is_not_closer(self) -> None:
        existing_entry = {
            "orderId": 84,
            "side": "SELL",
            "positionSide": "SHORT",
        ...
        guarded = apply_actual_net_exposure_decision_to_actions(
            actions={
                ...
                "cancel_orders": [dict(existing_entry)],
            },
            current_actual_net_qty=0.50,
            valuation_price=200.0,
            current_open_orders=[existing_entry],
    ...
    def test_cap_recovery_replaces_authorized_stale_balancing_entry(self) -> None:
        existing_entry = {
            "orderId": 85,
            "side": "SELL",
            "positionSide": "SHORT",
        ...
        guarded = apply_actual_net_exposure_decision_to_actions(
            actions={
                ...
                "cancel_orders": [],
                "same_side_spacing_guard": {
                    "protected_cancel_orders": [dict(existing_entry)],
                },
            },
            current_actual_net_qty=0.50,
            valuation_price=200.0,
        ...
        self.assertEqual(decision["stale_balancing_entry_price"], 203.0)
        self.assertEqual(decision["replacement_balancing_entry_price"], 201.0)
        self.assertEqual(guarded["cancel_orders"], [existing_entry])
        self.assertEqual(guarded["place_orders"], [])

    def test_cap_recovery_replaces_stale_entry_when_spacing_suppressed_candidate(self) -> None:
        existing_entry = {
            "orderId": 86,
            "side": "SELL",
            "positionSide": "SHORT",
        ...
        self.assertEqual(decision["stale_balancing_entry_price"], 0.2792)
        self.assertEqual(decision["replacement_balancing_entry_price"], 0.2791)
        self.assertEqual(guarded["cancel_orders"], [existing_entry])
        self.assertEqual(guarded["place_orders"], [])
    ...
    def test_reduce_only_no_loss_guard_keeps_authorized_frozen_manual_reduce(self) -> None:
        actions = {
            "place_orders": [
                {
                    "side": "SELL",
                    ...
                    "time_in_force": "IOC",
                    "manual_frozen_inventory_reduce": True,
                    "frozen_inventory_request_id": "manual-runtime-long-tp-1782187671811",
                },
                {
                    "side": "BUY",
                    "price": 0.8586,
        ...
        guarded = apply_reduce_only_no_loss_guard_to_actions(
            ...
            plan_report={
                ...
                "take_profit_guard": {
                    "enabled": True,
                    "effective_min_profit_ratio": 0.0,
                    "long_floor_price": 0.9159,
                    "short_ceiling_price": 0.823,
                },
                "current_long_avg_price": 0.9159,
                "current_short_avg_price": 0.823,
            },
            strategy_mode="hedge_best_quote_maker_volume_v1",
            live_bid_price=0.8586,
            live_ask_price=0.8587,
            tick_size=0.0001,
            min_qty=1.0,
            min_notional=5.0,
            step_size=1.0,
        ...
        self.assertEqual(guarded["place_count"], 1)
        self.assertEqual(guarded["place_orders"][0]["role"], "frozen_inventory_manual_reduce_long")
        self.assertEqual(
            guarded["place_orders"][0]["reduce_only_no_loss_guard"],
            "bypassed_frozen_inventory_manual_reduce",
        )
        dropped = guarded["reduce_only_no_loss_guard"]["dropped_orders"][0]
        self.assertEqual(dropped["role"], "best_quote_reduce_short")
        self.assertEqual(dropped["reduce_only_no_loss_drop_reason"], "short_reduce_above_no_loss_ceiling")
    ...
    def test_same_side_spacing_allows_replacing_cancelled_less_aggressive_entry(self) -> None:
        actions = {
            "place_orders": [{"side": "SELL", "price": 0.16006, "qty": 718.0, "notional": 114.92308, "role": "best_quote_entry_short"}],
            "cancel_orders": [{"orderId": 419310803, "side": "SELL", "price": "0.16014", "origQty": "717.0", "positionSide": "BOTH"}],
        }
        current_open_orders = [{"orderId": 419310803, "side": "SELL", "price": "0.16014", "origQty": "717.0", "positionSide": "BOTH"}]

        guarded = suppress_same_side_nearby_place_orders(
            actions=actions, current_open_orders=current_open_orders,
            min_price_spacing=0.00019, live_bid_price=0.15993, live_ask_price=0.15994,
            tick_size=0.00001, min_qty=1.0, min_notional=5.0, step_size=1.0,
            protect_cancelled_existing=False,
        )

        self.assertEqual(len(guarded["place_orders"]), 1)
        self.assertEqual(len(guarded["cancel_orders"]), 1)
