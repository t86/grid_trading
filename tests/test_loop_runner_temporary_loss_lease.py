...
def test_raw_allow_loss_cannot_bypass_invalid_typed_lease(
    ...
    assert not submit_roles or "best_quote_reduce_long" not in submit_roles
    assert not submit_roles or "best_quote_reduce_short" not in submit_roles
...
def test_submit_boundary_keeps_profitable_reduce_when_legacy_recovery_flag_is_on(
    tmp_path,
) -> None:
    args = Namespace(
        symbol="GRVTUSDT",
    ...
    guarded, reason = _guard_temporary_loss_order_before_submit(
        ...
        strategy_mode="hedge_best_quote_maker_volume_v1",
        live_bid_price=1.029,
        live_ask_price=1.030,
        tick_size=0.001,
        min_qty=1.0,
        min_notional=5.0,
        step_size=1.0,
    ...
    assert guarded["price"] == pytest.approx(1.03)
...
def test_submit_boundary_allows_one_active_pair_loss_only_above_fixed_threshold(
    ...
    above, above_reason = _guard_temporary_loss_order_before_submit(
        args=args,
        order=order,
        plan_report={
            "current_long_notional": 801.0,
    ...
    below, below_reason = _guard_temporary_loss_order_before_submit(
        args=args,
        order=order,
        plan_report={
