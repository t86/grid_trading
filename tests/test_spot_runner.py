...
class SpotRunnerTests(unittest.TestCase):
    ...
    def test_spot_runner_preflight_rejects_app_loss_window_mismatch(
        self,
        _mock_live_hedge,
    ) -> None:
        config = dict(SPOT_RUNNER_DEFAULT_CONFIG)
        config.update(
            {
                ...
                "spot_app_loss_prestart_gate_enabled": True,
                "spot_app_loss_prestart_gate_start_time": "2026-06-24T19:57:00+08:00",
                "runtime_guard_stats_start_time": "2026-06-20T08:00:00+08:00",
    ...
    @patch("grid_optimizer.web._spot_freeze_live_hedge_preflight_reason", return_value=None)
    def test_spot_runner_preflight_rejects_undersized_short_freeze_capacity(
        self,
        _mock_live_hedge,
    ) -> None:
        config = dict(SPOT_RUNNER_DEFAULT_CONFIG)
        config.update(
            {
                ...
                "spot_freeze_total_cap_notional": 240.0,
                "spot_freeze_max_per_cycle_notional": 60.0,
                "max_short_position_notional": 30.0,
            }
        )
        ...
        self.assertIn("max_short_position_notional", str(raised.exception))
        self.assertIn("spot_freeze", str(raised.exception))

    @patch("grid_optimizer.web._spot_freeze_live_hedge_preflight_reason", return_value=None)
    def test_spot_runner_preflight_rejects_undersized_spot_freeze_total_cap(
        self,
        _mock_live_hedge,
    ) -> None:
        config = dict(SPOT_RUNNER_DEFAULT_CONFIG)
        config.update(
            {
                ...
                "spot_freeze_base_hedge_qty": 4800.0,
                "spot_freeze_deviation_notional": 60.0,
                "spot_freeze_total_cap_notional": 60.0,
        ...
        self.assertIn("spot_freeze_total_cap_notional", str(raised.exception))
        self.assertIn("max_short_position_notional", str(raised.exception))
