...
class LoopRunnerTests(unittest.TestCase):
    ...
    def test_best_quote_reduce_freeze_can_disable_long_side_only(self) -> None:
        state: dict[str, object] = {
            "best_quote_volume_ledger": {
                "long_lots": [{"qty": 1.0, "price": 120.0}],
                "short_lots": [{"qty": 1.0, "price": 100.0}],
                "long_qty": 1.0,
                "short_qty": 1.0,
                "long_avg_price": 120.0,
                "short_avg_price": 100.0,
                "initialized": True,
            },
        }
        report = _best_quote_reduce_freeze_report(
            ...
            current_long_qty=1.0,
            current_short_qty=1.0,
            current_long_avg_price=120.0,
            current_short_avg_price=100.0,
            mid_price=110.0,
            bq_ledger_report=state["best_quote_volume_ledger"],
        )
        report = _apply_best_quote_reduce_freeze(
        ...
        self.assertTrue(report["freeze_side_gate"]["long"]["enabled"] is False)
        self.assertEqual(report["freeze_side_gate"]["long"]["blocked_reason"], "side_disabled")
        self.assertEqual(report["frozen_long_qty"], 0.0)
        self.assertGreater(report["frozen_short_qty"], 0.0)
    ...
    @patch("grid_optimizer.loop_runner.fetch_futures_book_tickers")
    @patch("grid_optimizer.loop_runner.fetch_futures_symbol_config")
    def test_best_quote_cost_gate_preserves_hedge_entry_when_lossless_reduce_is_impossible(
        self,
        mock_symbol_config,
        mock_book_tickers,
        mock_premium_index,
        ...
        mock_account_info.return_value = {
            "multiAssetsMargin": False,
            "positions": [
                {"symbol": "PHAROSUSDT", "positionSide": "LONG", "positionAmt": "23", "entryPrice": "0.8000"},
                {"symbol": "PHAROSUSDT", "positionSide": "SHORT", "positionAmt": "-74", "entryPrice": "0.6630"},
            ],
        }
        mock_open_orders.return_value = []
        ...
        gate = report["best_quote_maker_volume"]["inventory_cost_gate"]
        self.assertEqual(report["sell_orders"][0]["role"], "best_quote_entry_short")
        self.assertFalse(report["sell_orders"][0].get("force_reduce_only", False))
        self.assertEqual(gate["reduce_fallback"]["loss_protected_fallback_bypasses"], 1)
    ...
    def test_loss_reduce_reentry_guard_prefers_lossy_active_pair_side(self) -> None:
        guard = resolve_loss_reduce_reentry_guard(
            ...
            best_quote_active_pair_reduce={
                "enabled": True,
                "order_count": 2,
                "long_order_notional": 99.99,
                "short_order_notional": 99.96,
                "long_expected_loss_ratio": 0.0,
                "short_expected_loss_ratio": 0.0055,
            },
            current_long_notional=944.0,
            current_short_notional=943.0,
            mid_price=0.29845,
            effective_step_price=0.00035,
            now=datetime(2026, 8, 15, 2, 7, tzinfo=timezone.utc),
            cooldown_seconds=300.0,
            recover_buffer_steps=1.0,
        )
    ...
    def test_best_quote_active_pair_reduce_threshold_mode_reduces_only_heavy_side(self) -> None:
        plan = {
            "buy_orders": [
        ...
        report = apply_best_quote_active_pair_reduce(
            ...
            state={},
            enabled=True,
            current_long_qty=3138.0,
            current_short_qty=1287.0,
            current_long_notional=907.0,
            current_short_notional=372.0,
            max_long_notional=1000.0,
            max_short_notional=1000.0,
            soft_ratio=0.90,
            min_side_notional=0.0,
        ...
        self.assertAlmostEqual(report["long_target_notional"], 845.0)
        self.assertNotIn("best_quote_entry_long", [item["role"] for item in plan["buy_orders"]])
        self.assertIn("best_quote_entry_short", [item["role"] for item in plan["sell_orders"]])
        self.assertIn(
            "best_quote_active_pair_reduce_long",
            [item["role"] for item in plan["sell_orders"]],
        )
    ...
    def test_grvt_pressure_guard_applies_when_active_pair_reduce_is_disabled(self) -> None:
        ...
        self.assertTrue(report["active"])
        self.assertEqual(report["eligible_sides"], ["short"])
        self.assertEqual(report["suppressed_opposite_reduce_order_count"], 1)
    ...
    def test_best_quote_active_pair_reduce_threshold_mode_does_not_reduce_below_threshold(self) -> None:
        ...
        report = apply_best_quote_active_pair_reduce(
            ...
            state={},
            enabled=True,
            current_long_qty=2800.0,
            current_short_qty=1200.0,
            current_long_notional=799.0,
            current_short_notional=340.0,
            max_long_notional=1000.0,
            max_short_notional=1000.0,
            soft_ratio=0.70,
            min_side_notional=0.0,
        ...
        self.assertFalse(report["active"])
        self.assertEqual(report["reason"], "below_soft")
        self.assertEqual(plan["buy_orders"], [])
        self.assertEqual(plan["sell_orders"], [])
    ...
    def test_paired_threshold_lease_does_not_restore_budget_after_late_entry_fill(self) -> None:
        ...
        self.assertLessEqual(first["short_order_notional"], 100.0)
        first_long_authorized = first["long_authorized_reduce_notional"]
        ...
        self.assertEqual(
            second["long_authorized_reduce_notional"],
            first_long_authorized,
        )
        self.assertEqual(second_plan["sell_orders"], [])
        self.assertEqual(
    ...
    def test_paired_threshold_lease_refreshes_unfilled_pressure_order(self) -> None:
        now = datetime(2026, 8, 17, 8, 20, tzinfo=timezone.utc)
        state: dict[str, Any] = {}
        common = {
            "state": state,
            "enabled": True,
            "current_long_qty": 3351.0,
            "current_short_qty": 3077.0,
            "current_long_notional": 947.8,
            "current_short_notional": 870.5,
            "max_long_notional": 1000.0,
            "max_short_notional": 1000.0,
            "soft_ratio": 0.90,
            "min_side_notional": 0.0,
            ...
            "volatility_entry_pause": {"active": False},
            "loss_reduce_threshold_notional": 900.0,
            "current_long_avg_price": 0.2855,
            "current_short_avg_price": 0.2817,
            "max_loss_ratio": 0.20,
            "min_relief_notional": 100.0,
            "pair_all_sides_on_threshold": True,
            "rearm_cooldown_seconds": 60.0,
        ...
        self.assertEqual(second_plan["sell_orders"][0]["price"], 0.2822)
        self.assertEqual(
            second["long_authorized_reduce_notional"],
            first_long_authorized,
        )
    ...
    def test_grvt_bounded_loss_recovery_clears_one_hundred_below_soft(self) -> None:
        ...
        report = apply_best_quote_active_pair_reduce(
            ...
            state={},
            enabled=True,
            current_long_qty=3305.0,
            current_short_qty=2997.0,
            current_long_notional=931.5,
            current_short_notional=844.7,
            max_long_notional=1000.0,
            max_short_notional=1000.0,
            soft_ratio=0.90,
            min_side_notional=0.0,
            ...
            volatility_entry_pause={"active": False},
            loss_reduce_threshold_notional=900.0,
            current_long_avg_price=0.297245,
    ...
    def test_threshold_mode_completion_rearms_when_another_ordinary_side_crosses_threshold(self) -> None:
        ...
        self.assertEqual(first["eligible_sides"], ["long"])
        second_plan = {
            "buy_orders": [],
            "sell_orders": [
                {"side": "SELL", "role": "best_quote_entry_short", "notional": 55.0}
    ...
    def test_threshold_mode_waits_five_minutes_before_rearming_same_side(self) -> None:
        ...
        self.assertFalse(report["active"])
        self.assertEqual(report["reason"], "rearm_cooldown_active")
        self.assertGreater(report["rearm_cooldown_remaining_seconds"], 200.0)
        self.assertTrue(report["normal_entry_suppressed"])
        self.assertEqual(plan["sell_orders"], [])
    ...
    def test_threshold_mode_completion_cycle_blocks_same_side_refill(self) -> None:
        state = {
            "best_quote_active_pair_reduce": {
                ...
                "mode": "threshold_side_v1",
                "eligible_sides": ["short"],
                "short_start_notional": 960.0,
                "short_target_notional": 860.0,
                "long_start_notional": 800.0,
                "long_target_notional": 800.0,
                "trigger_count": 1,
            }
        }
        plan = {
        ...
        report = apply_best_quote_active_pair_reduce(
            ...
            state=state,
            enabled=True,
            current_long_qty=2300.0,
            current_short_qty=2470.0,
            current_long_notional=800.0,
            current_short_notional=858.0,
            max_long_notional=1000.0,
            max_short_notional=1000.0,
            soft_ratio=0.90,
            min_side_notional=0.0,
            ...
            volatility_entry_pause={"active": False},
            loss_reduce_threshold_notional=900.0,
            current_long_avg_price=0.3450,
            current_short_avg_price=0.3400,
            max_loss_ratio=0.20,
            min_relief_notional=100.0,
            suppress_all_entries_while_active=True,
            now=datetime(2026, 8, 14, 11, 49, tzinfo=timezone.utc),
            rearm_cooldown_seconds=300.0,
        )

        self.assertEqual(report["reason"], "target_reached")
        self.assertTrue(report["completed"])
        self.assertTrue(report["normal_entry_suppressed"])
        self.assertEqual(plan["sell_orders"], [])
    ...
    def test_threshold_mode_disable_releases_entry_below_threshold(self) -> None:
        ...
        report = apply_best_quote_active_pair_reduce(
            ...
            enabled=False,
            current_long_qty=2500.0,
            current_short_qty=2800.0,
            current_long_notional=800.0,
            current_short_notional=880.0,
            max_long_notional=1000.0,
            max_short_notional=1000.0,
            soft_ratio=0.90,
            min_side_notional=0.0,
