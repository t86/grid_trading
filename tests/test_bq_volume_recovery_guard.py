...
class BqVolumeRecoveryGuardTests(unittest.TestCase):
    ...
    def test_registered_round_blocks_when_snapshot_restore_write_fails(self) -> None:
        with TemporaryDirectory() as tmpdir:
            output_dir = Path(tmpdir)
            control_path = output_dir / "bchusdt_loop_runner_control.json"
            now = datetime(2026, 7, 16, tzinfo=timezone.utc)
    ...
    def test_arx_severe_pace_capacity_raises_thresholds_and_budget_only_when_calm(self) -> None:
        ...
        self.assertNotIn("max_actual_net_notional", updates)
    ...
    def test_arx_side_cap_unwind_prioritizes_larger_excess_and_resets(self) -> None:
        ...
        self.assertTrue(
            bq_volume_recovery_guard.should_bypass_arx_side_unwind_recovery_gate(
                symbol="ARXUSDT",
                updates={"best_quote_maker_volume_directional_net_guard": "off"},
                control={"best_quote_maker_volume_directional_net_guard": "net_short"},
            )
        )
        ...
        soft_limit_start = bq_volume_recovery_guard.arx_side_cap_unwind_updates(
            ...
            actual_long_notional=1988.0,
            actual_short_notional=129.0,
        )
        self.assertEqual("net_long", soft_limit_start["best_quote_maker_volume_directional_net_guard"])

        net_limit_start = bq_volume_recovery_guard.arx_side_cap_unwind_updates(
            control={
                "max_actual_net_notional": 1000.0,
                "best_quote_maker_volume_directional_net_guard": "off",
                "best_quote_maker_volume_allow_loss_reduce_only": False,
            },
            actual_long_notional=1355.0,
            actual_short_notional=129.0,
        )
        ...
        normal_net_ceiling_still_recovers_to_balance = (
    ...
    def test_arx_side_cap_unwind_keeps_bounded_cap_relief_during_high_wear(self) -> None:
        cap_relief = bq_volume_recovery_guard.arx_side_cap_unwind_updates(
            control={
                "max_actual_net_notional": 1000.0,
                "max_position_notional": 800.0,
                "max_short_position_notional": 800.0,
        ...
        self.assertEqual("net_long", cap_relief["best_quote_maker_volume_directional_net_guard"])
        self.assertTrue(cap_relief["best_quote_maker_volume_allow_loss_reduce_only"])
        self.assertFalse(cap_relief["best_quote_maker_volume_net_loss_reduce_enabled"])

        continued_relief = bq_volume_recovery_guard.arx_side_cap_unwind_updates(
            control={
                "max_actual_net_notional": 1000.0,
        ...
        self.assertEqual(
            "net_short",
            continued_relief["best_quote_maker_volume_directional_net_guard"],
        )
        self.assertNotEqual(
            False,
            continued_relief.get("best_quote_maker_volume_allow_loss_reduce_only", True),
        )
    ...
    def test_arx_severe_pace_capacity_lowers_previously_overridden_side_cap(self) -> None:
        updates = bq_volume_recovery_guard.arx_severe_pace_capacity_updates(
            ...
            volatility_entry_pause_active=False,
            frozen_total_notional=400.0,
            actual_long_notional=2818.0,
            actual_short_notional=4133.0,
        )
        self.assertEqual(2000.0, updates["max_position_notional"])
        self.assertEqual(2000.0, updates["max_short_position_notional"])
        self.assertEqual(4000.0, updates["max_total_notional"])
        ...
        self.assertEqual(
            ...
            bq_volume_recovery_guard.arx_severe_pace_capacity_updates(
                control={
                    ...
                    "max_position_notional": 2000.0,
                    "max_short_position_notional": 2000.0,
                    "max_actual_net_notional": 5000.0,
                    "maker_max_long_notional": 2000.0,
                    "maker_max_short_notional": 2000.0,
                    "best_quote_maker_volume_max_long_notional": 2000.0,
                    "best_quote_maker_volume_max_short_notional": 2000.0,
    ...
    def test_arx_exchange_order_drift_restarts_only_when_exchange_is_empty(self) -> None:
        ...
        recent_activity = bq_volume_recovery_guard.recover_arx_exchange_order_drift(
            ...
            submit={
                "observed_execution_events": [
                    {"kind": "ORDER_NEW", "event_time": int(now.timestamp() * 1000)}
                ]
            },
            state={},
            now=now + timedelta(seconds=10),
        ...
        self.assertEqual(["ARXUSDT"], restarted)
        partial_recent_activity = bq_volume_recovery_guard.recover_arx_exchange_order_drift(
            symbol="ARXUSDT",
            local_active_order_count=2,
            expected_entry_order_count=4,
            submit={
            ...
            exchange_snapshot_fetcher=lambda _symbol: {
                "strategy_open_order_count": 2,
                "strategy_order_ids": ["123", "124"],
            },
        )
        self.assertTrue(partial_recent_activity["allow_recovery_check"])
        self.assertIsNone(
    ...
    def test_arx_independent_freeze_policy_is_stable_at_expected_values(self) -> None:
        control = {
            ...
            "best_quote_maker_volume_frozen_long_cap_notional": 400.0,
            "best_quote_maker_volume_frozen_short_cap_notional": 400.0,
            "best_quote_maker_volume_reduce_freeze_quality_gate_enabled": True,
            "best_quote_maker_volume_reduce_freeze_quality_max_loss_ratio": 0.03,
            "best_quote_maker_volume_reduce_freeze_quality_release_profit_ratio": 0.02,
    ...
    def test_main_defers_long_exchange_cooldown_without_failing_guard(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            with (
                ...
                patch.object(
                    ...
                    side_effect=RuntimeError(
                        "Binance API local cooldown active for futures; retry after 33.6s"
                    ),
                ),
                redirect_stdout(stdout),
            ):
            ...
            self.assertEqual(result["action"], "defer_exchange_trade_fetch_cooldown")
            self.assertEqual(result["retry_after_seconds"], 33.6)
            state = json.loads(state_path.read_text(encoding="utf-8"))
    ...
    def test_inactive_runner_recovers_net_long_guard_stop_with_directional_orders_only(self) -> None:
    ...
    def test_inactive_runner_recovers_net_short_guard_stop_without_short_reduce_suppression(self) -> None:
        now = datetime(2026, 7, 13, 1, 2, tzinfo=timezone.utc)
        with TemporaryDirectory() as tmpdir:
            output_dir = Path(tmpdir)
            self._write_common_files(
                output_dir,
    ...
    def _write_common_files(
        ...
        *,
        ...
        output_prefix = normalized_symbol.lower()
        control_payload = {
        ...
        _write_json(
            output_dir / f"{output_prefix}_loop_runner_control.json",
            control_payload,
        )
        ...
        _write_json(
            output_dir / f"{output_prefix}_loop_latest_plan.json",
            {
        ...
        _write_json(
            output_dir / f"{output_prefix}_loop_latest_submit.json",
            {
                "generated_at": now.isoformat(),
        ...
        _append_jsonl(
            output_dir / f"{output_prefix}_loop_trade_audit.jsonl",
            rows,
        )
    ...
    def test_soft_recovery_clears_stale_short_reduce_suppression(self) -> None:
        now = datetime(2026, 7, 13, 9, 40, tzinfo=timezone.utc)
        with TemporaryDirectory() as tmpdir:
            output_dir = Path(tmpdir)
            self._write_common_files(
                output_dir,
            ...
            plan["pause_reasons"] = ["inventory_soft", "low_volatility_tighten"]
            _write_json(plan_path, plan)
            state: dict[str, object] = {
                "symbols": {
                    "REUSDT": {
            ...
            self.assertNotIn(
                "best_quote_maker_volume_suppress_short_reduce_enabled",
                state["symbols"]["REUSDT"]["guard_original_controls"],
            )

    def test_active_soft_recovery_clears_stale_short_reduce_suppression(self) -> None:
        now = datetime(2026, 7, 13, 9, 45, tzinfo=timezone.utc)
        with TemporaryDirectory() as tmpdir:
            output_dir = Path(tmpdir)
            self._write_common_files(
                output_dir,
            ...
            state: dict[str, object] = {
                "symbols": {
                    "REUSDT": {
                        "status": "recovery_active",
                        "recovery_owned": True,
                        "recovery_started_at": (now - timedelta(seconds=30)).isoformat(),
                        "guard_original_controls": {
                            "best_quote_maker_volume_allow_loss_reduce_only": False,
                        },
                    }
    ...
    def test_arx_target_miss_keeps_loss_reduce_at_best_quote(self) -> None:
        now = datetime(2026, 7, 12, 4, 16, tzinfo=timezone.utc)
        with TemporaryDirectory() as tmpdir:
            output_dir = Path(tmpdir)
            self._write_common_files(
            ...
            for suffix in (
                "loop_runner_control.json",
                "loop_latest_plan.json",
                "loop_latest_submit.json",
                "loop_trade_audit.jsonl",
            ):
                (output_dir / f"reusdt_{suffix}").rename(output_dir / f"arxusdt_{suffix}")
            ...
            result = check_symbol(
                ...
                min_volume_notional=100,
                trigger_seconds=0,
                daily_target_notional=180000.0,
                target_pace_fraction=1.05,
                loss_reduce_quote_offset_extra_ticks=1,
                restart_runner=lambda _symbol: None,
            )

            control = json.loads((output_dir / "arxusdt_loop_runner_control.json").read_text(encoding="utf-8"))
    ...
    def test_wrong_way_recovery_keeps_budget_on_available_opposite_entry(self) -> None:
        now = datetime(2026, 7, 12, 4, 22, tzinfo=timezone.utc)
        with TemporaryDirectory() as tmpdir:
            output_dir = Path(tmpdir)
            self._write_common_files(
                output_dir,
    ...
    def test_high_wear_suppresses_restored_active_pair_reduce(self) -> None:
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertFalse(result["assessment"]["target_pace_behind"])
            self.assertTrue(result["assessment"]["confirmed_loss_reduce_wear"])
            self.assertFalse(control["best_quote_maker_volume_allow_loss_reduce_only"])
            self.assertFalse(control["best_quote_maker_volume_net_loss_reduce_enabled"])
            self.assertFalse(control["best_quote_maker_volume_active_pair_reduce_enabled"])
    ...
    def test_grvt_two_sided_entry_pace_ramp_advances_budget_by_one_step(self) -> None:
        now = datetime(2026, 8, 15, 1, 0, tzinfo=timezone.utc)
        with TemporaryDirectory() as tmpdir:
            output_dir = Path(tmpdir)
            self._write_common_files(
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            result = check_symbol(
                ...
                trade_rows=[
                    ...
                    {
                        "id": 2,
                        "time": int((now - timedelta(minutes=8)).timestamp() * 1000),
                        "quoteQty": "400",
                        "realizedPnl": "0",
                    },
                ],
            ...
            self.assertTrue(result["assessment"]["high_recovery_wear"])
            self.assertTrue(
                result["assessment"]["grvt_transient_completed_loss_release_wear"]
            )
            self.assertEqual(
                result["action"],
                "ramp_grvt_two_sided_entry_after_completed_loss_release",
            )
            self.assertEqual(control["best_quote_maker_volume_cycle_budget_notional"], 77.0)
            self.assertFalse(
                control.get("best_quote_maker_volume_allow_loss_reduce_only", False)
            )
            self.assertEqual(control["pause_buy_position_notional"], 900.0)
            self.assertEqual(control["pause_short_position_notional"], 900.0)
            self.assertEqual(restarts, ["GRVTUSDT"])
    ...
    def test_grvt_volatility_pause_does_not_enable_loss_recovery(self) -> None:
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            plan.update(
                {
                    ...
                    "pause_reasons": ["extreme_volatility_defensive"],
                    "short_pause_reasons": ["extreme_volatility_defensive"],
                }
            )
            _write_json(plan_path, plan)
            state: dict[str, object] = {
            ...
            self.assertEqual(
                result["action"],
                "hold_grvt_volatility_pause_without_loss_recovery",
            )
            self.assertFalse(
                control["best_quote_maker_volume_allow_loss_reduce_only"]
            )
    ...
    def test_grvt_dynamic_high_allows_bounded_release_when_both_sides_exceed_soft(self) -> None:
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            plan.update(
                {
                    "volatility_entry_pause": {"active": False},
                    "pause_reasons": ["high_volatility_defensive", "inventory_soft"],
                    "short_pause_reasons": ["high_volatility_defensive", "inventory_soft"],
                    "buy_orders": [
                        {"role": "best_quote_reduce_short", "price": 0.36, "notional": 100.0}
                    ],
                    "sell_orders": [
                        {"role": "best_quote_reduce_long", "price": 0.37, "notional": 100.0}
                    ],
                }
            )
            _write_json(plan_path, plan)
    ...
    def test_grvt_dynamic_extreme_allows_bounded_release_when_one_side_exceeds_soft(self) -> None:
        now = datetime(2026, 8, 10, 12, 50, tzinfo=timezone.utc)
        with TemporaryDirectory() as tmpdir:
            output_dir = Path(tmpdir)
            self._write_common_files(
                output_dir,
            ...
            plan.update(
                {
                    "volatility_entry_pause": {"active": False},
                    "pause_reasons": ["extreme_volatility_defensive", "inventory_soft"],
                    "short_pause_reasons": ["extreme_volatility_defensive"],
                    "buy_orders": [],
                    "sell_orders": [
                        {
    ...
    def test_grvt_recovery_guard_read_only_forces_dry_run(self) -> None:
        now = datetime(2026, 8, 15, 16, 45, tzinfo=timezone.utc)
        with TemporaryDirectory() as tmpdir:
            output_dir = Path(tmpdir)
            self._write_common_files(
                output_dir,
                ...
                symbol="GRVTUSDT",
                control={
                    "best_quote_maker_volume_recovery_guard_read_only": True,
                    "best_quote_maker_volume_active_pair_reduce_enabled": False,
                    "best_quote_maker_volume_active_pair_reduce_order_notional": 55.0,
                    "best_quote_maker_volume_active_pair_reduce_max_notional_per_side": 100.0,
                    "best_quote_maker_volume_cycle_budget_notional": 260.0,
            ...
            _write_json(plan_path, plan)
            control_path = output_dir / "grvtusdt_loop_runner_control.json"
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            self._write_common_files(
                ...
                control={
                    ...
                    "best_quote_maker_volume_active_pair_reduce_enabled": True,
                    "best_quote_maker_volume_cycle_budget_notional": 250.0,
                    "best_quote_maker_volume_quote_offset_ticks": 6,
                    "pause_buy_position_notional": 900.0,
                    "pause_short_position_notional": 900.0,
                },
                long_notional=895.0,
            ...
            self.assertEqual(
                result["action"],
                "disable_grvt_loss_recovery_during_volatility_pause",
            )
            self.assertFalse(
                control["best_quote_maker_volume_allow_loss_reduce_only"]
            )
    ...
    def test_grvt_dynamic_extreme_keeps_owned_release_until_two_sided_flow(self) -> None:
        now = datetime(2026, 8, 10, 12, 20, tzinfo=timezone.utc)
        with TemporaryDirectory() as tmpdir:
            output_dir = Path(tmpdir)
            self._write_common_files(
                output_dir,
            ...
            submit["observed_strategy_open_order_state"].update(
                {
                    "ordinary_active_entry_long_order_count": 0,
                    "ordinary_active_entry_short_order_count": 2,
                }
            )
            _write_json(submit_path, submit)
            state: dict[str, object] = {
            ...
            self.assertEqual(
                result["action"],
                "hold_grvt_dynamic_loss_release_until_two_sided_flow",
            )
            self.assertTrue(control["best_quote_maker_volume_allow_loss_reduce_only"])
            self.assertTrue(control["best_quote_maker_volume_active_pair_reduce_enabled"])
            self.assertEqual(restarts, [])

    def test_grvt_dynamic_release_restores_two_sided_flow_and_tightens_offset(self) -> None:
    ...
    def test_grvt_dynamic_release_restores_when_one_balancing_entry_is_live(self) -> None:
        now = datetime(2026, 8, 15, 3, 10, tzinfo=timezone.utc)
        with TemporaryDirectory() as tmpdir:
            output_dir = Path(tmpdir)
            self._write_common_files(
                output_dir,
            ...
            plan["buy_orders"] = [
                {"role": "best_quote_entry_long", "notional": 25.0}
            ]
            plan["sell_orders"] = []
            _write_json(plan_path, plan)
            submit_path = output_dir / "grvtusdt_loop_latest_submit.json"
    ...
    def test_grvt_loss_lease_below_soft_without_entries_restores_entry(self) -> None:
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            plan.update(
                {
                    "volatility_entry_pause": {"active": False},
                    "pause_reasons": ["high_volatility_defensive"],
                    "best_quote_active_pair_reduce": {
                        "enabled": True,
                        "active": True,
                        "completed": False,
    ...
    def test_grvt_loss_lease_below_soft_one_sided_plan_without_live_entries_restores(self) -> None:
        now = datetime(2026, 8, 15, 9, 10, tzinfo=timezone.utc)
        with TemporaryDirectory() as tmpdir:
            output_dir = Path(tmpdir)
            self._write_common_files(
                output_dir,
                ...
                long_notional=878.0,
                short_notional=834.0,
                open_order_count=0,
                active_order_count=0,
            )
            plan_path = output_dir / "grvtusdt_loop_latest_plan.json"
            plan = json.loads(plan_path.read_text(encoding="utf-8"))
            plan["buy_orders"] = [{"role": "best_quote_entry_long", "notional": 55.0}]
            plan["sell_orders"] = []
            _write_json(plan_path, plan)
            submit_path = output_dir / "grvtusdt_loop_latest_submit.json"
            submit = json.loads(submit_path.read_text(encoding="utf-8"))
            ...
            self.assertEqual(result["action"], "restore_grvt_after_dynamic_release_below_soft")
            self.assertFalse(control["best_quote_maker_volume_allow_loss_reduce_only"])
            self.assertFalse(control["best_quote_maker_volume_active_pair_reduce_enabled"])
            self.assertEqual(restarts, ["GRVTUSDT"])
            ...
            plan.update(
                {
                    ...
                    "best_quote_active_pair_reduce": {
                        ...
                        "order_count": 0,
                        "suppressed_entry_order_count": 2,
                        "short_blocked_reason": "loss_ratio_above_limit",
                    },
                    "pause_reasons": ["volatility_entry_pause", "inventory_soft"],
                    "short_pause_reasons": ["volatility_entry_pause", "inventory_soft"],
                }
    ...
    def test_active_grvt_inventory_relief_pulls_far_balancing_entry_closer(self) -> None:
        now = datetime(2026, 8, 9, 9, 35, tzinfo=timezone.utc)
        with TemporaryDirectory() as tmpdir:
            output_dir = Path(tmpdir)
            self._write_common_files(
                output_dir,
                ...
                symbol="GRVTUSDT",
                control={
                    "best_quote_maker_volume_cycle_budget_notional": 204.0,
                    "best_quote_maker_volume_quote_offset_ticks": 3,
                    "pause_buy_position_notional": 900.0,
                    "pause_short_position_notional": 900.0,
                    "sticky_entry_price_tolerance_steps": 1.0,
            ...
            self.assertEqual(
                result["action"],
                "pull_active_grvt_balancing_entry_one_tick_closer",
            )
            self.assertEqual(control["best_quote_maker_volume_quote_offset_ticks"], 2)
            self.assertEqual(restarts, ["GRVTUSDT"])
