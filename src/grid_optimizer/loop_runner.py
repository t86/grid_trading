...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--exposure-escalation-hard-unrealized-loss-limit", type=float, default=10.0)
    ...
    try:
        while True:
            ...
            try:
                ...
                "strategy_profile_schema": dict(plan_report.get("strategy_profile_schema") or {}),
                "strategy_profile_schema_strict": bool(
                    (plan_report.get("strategy_profile_schema") or {}).get("strict_enabled")
                ),
                "strategy_intent": str(plan_report.get("strategy_intent") or ""),
                "active_state": str(plan_report.get("active_state") or ""),
                "state_reason": list(plan_report.get("state_reason") or []),
                "allowed_sides": list(plan_report.get("allowed_sides") or []),
                "ignored_params": list(plan_report.get("ignored_params") or []),
                "repair_ladder_level": str((plan_report.get("elastic_volume") or {}).get("repair_ladder_level") or ""),
                "repair_stale_cycles": int(
                    _safe_float(((plan_report.get("elastic_volume") or {}).get("repair_ladder") or {}).get("stale_cycles"))
                ),
                "adverse_move_ticks": _safe_float(
                    ((plan_report.get("elastic_volume") or {}).get("repair_ladder") or {}).get("adverse_move_ticks")
                ),
                "adverse_move_bps": _safe_float(
                    ((plan_report.get("elastic_volume") or {}).get("repair_ladder") or {}).get("adverse_move_bps")
                ),
                "repair_slice_notional": _safe_float(
                    ((plan_report.get("elastic_volume") or {}).get("repair_ladder") or {}).get("repair_slice_notional")
                ),
                "repair_loss_budget_per_10k": _safe_float(
                    ((plan_report.get("elastic_volume") or {}).get("repair_ladder") or {}).get("max_repair_loss_per_10k")
                ),
                "best_quote_repair_ladder": dict(plan_report.get("best_quote_repair_ladder") or {}),
                "best_quote_repair_active": bool((plan_report.get("best_quote_repair_ladder") or {}).get("active")),
                "best_quote_repair_level": str((plan_report.get("best_quote_repair_ladder") or {}).get("level") or ""),
                "best_quote_repair_side": str((plan_report.get("best_quote_repair_ladder") or {}).get("side") or ""),
                "best_quote_repair_notional": _safe_float(
                    (plan_report.get("best_quote_repair_ladder") or {}).get("placed_repair_notional")
                ),
                "best_quote_repair_dropped_entries": int(
                    _safe_float((plan_report.get("best_quote_repair_ladder") or {}).get("dropped_entry_order_count"))
                ),
