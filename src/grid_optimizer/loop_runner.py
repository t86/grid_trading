...
def _resolve_long_cost_basis_price(*prices: float) -> float:
    candidates = [max(_safe_float(price), 0.0) for price in prices]
    positives = [price for price in candidates if price > 0]
    return max(positives) if positives else 0.0


def _resolve_short_cost_basis_price(*prices: float) -> float:
    candidates = [max(_safe_float(price), 0.0) for price in prices]
    positives = [price for price in candidates if price > 0]
...
def _planner_namespace(args: argparse.Namespace) -> argparse.Namespace:
    return argparse.Namespace(
        ...
        near_market_entry_max_center_distance_steps=getattr(args, "near_market_entry_max_center_distance_steps", 2.0),
        grid_inventory_rebalance_min_center_distance_steps=getattr(
            args,
            "grid_inventory_rebalance_min_center_distance_steps",
            3.0,
        ),
        near_market_reentry_confirm_cycles=getattr(args, "near_market_reentry_confirm_cycles", 2),
...
def _fetch_trade_rows_since(
    *,
    ...
    return fetch_time_paged_by_windows(
...
def apply_take_profit_profit_guard(
    *,
    ...
    long_has_position = safe_long_qty > 1e-12
    short_has_position = safe_short_qty > 1e-12
    long_active = long_has_position and safe_long_avg_price > 0
    short_active = short_has_position and safe_short_avg_price > 0
    long_cost_basis_missing = long_has_position and safe_long_avg_price <= 0
    short_cost_basis_missing = short_has_position and safe_short_avg_price <= 0
        ...
        *,
        ...
        for item in plan.get(order_key, []):
            if not isinstance(item, dict):
                continue
            order = dict(item)
            if _order_role(order) in guarded_roles and not _truthy(order.get("lot_tracked")):
    ...
    long_floor_price = None
    if long_active:
        long_floor_price = _round_order_price(safe_long_avg_price * (1.0 + safe_ratio), tick_size, "SELL")
        if long_floor_price > 0:
            report["long_floor_price"] = long_floor_price

    short_ceiling_price = None
    if short_active:
        short_ceiling_price = _round_order_price(safe_short_avg_price * (1.0 - safe_ratio), tick_size, "BUY")
...
def _synthetic_active_delever_guard_reason(
    *,
    side: str,
    current_qty: float,
    current_avg_price: float,
    unmatched_trade_count: int | float | None,
) -> str | None:
    normalized_side = str(side or "").strip().lower()
    qty = max(float(current_qty), 0.0)
    avg_price = max(float(current_avg_price), 0.0)
    unmatched = max(int(unmatched_trade_count or 0), 0)
    if unmatched > 0:
        return "synthetic_unmatched_trades"
    if qty > 1e-12 and avg_price <= 0:
        if normalized_side == "long":
            return "synthetic_long_avg_price_invalid"
        if normalized_side == "short":
            return "synthetic_short_avg_price_invalid"
        return "synthetic_avg_price_invalid"
    return None


def resolve_synthetic_distance_regime(
    *,
    state: dict[str, Any],
    center_price: float,
    mid_price: float,
    step_price: float,
    near_market_max_center_distance_steps: float,
    grid_inventory_rebalance_min_center_distance_steps: float,
    near_market_reentry_confirm_cycles: int,
) -> dict[str, Any]:
    safe_step_price = max(float(step_price), 1e-12)
    near_threshold = max(float(near_market_max_center_distance_steps), 0.0)
    rebalance_threshold = max(float(grid_inventory_rebalance_min_center_distance_steps), near_threshold)
    confirm_cycles = max(int(near_market_reentry_confirm_cycles), 1)
    distance_steps = abs(float(center_price) - float(mid_price)) / safe_step_price
    side_epsilon = safe_step_price * 1e-6
    if float(mid_price) < float(center_price) - side_epsilon:
        displacement_side = "long"
    elif float(mid_price) > float(center_price) + side_epsilon:
        displacement_side = "short"
    else:
        displacement_side = "flat"

    regime_state = dict(state.get("synthetic_distance_regime_state") or {})
    near_market_active = bool(regime_state.get("near_market_active", True))
    reentry_pending_count = int(regime_state.get("reentry_pending_count") or 0)
    switched = False

    if distance_steps > near_threshold + 1e-12:
        if near_market_active:
            switched = True
        near_market_active = False
        reentry_pending_count = 0
    elif near_market_active:
        reentry_pending_count = 0
    else:
        reentry_pending_count += 1
        if reentry_pending_count >= confirm_cycles:
            near_market_active = True
            reentry_pending_count = 0
            switched = True

    inventory_rebalance_active = (
        displacement_side in {"long", "short"}
        and distance_steps >= rebalance_threshold - 1e-12
    )
    target_inventory_levels = int(math.floor(distance_steps + 1e-12)) if inventory_rebalance_active else 0
    zone = "near_market" if near_market_active and distance_steps <= near_threshold + 1e-12 else (
        "grid_rebalance" if inventory_rebalance_active else "grid_transition"
    )
    regime_state.update(
        {
            "near_market_active": near_market_active,
            "reentry_pending_count": reentry_pending_count,
    ...
    state["synthetic_distance_regime_state"] = regime_state
    return {
        "enabled": True,
        "distance_steps": distance_steps,
        "zone": zone,
        "displacement_side": displacement_side,
        "near_market_active": near_market_active,
        "near_market_threshold_steps": near_threshold,
        "grid_inventory_rebalance_min_center_distance_steps": rebalance_threshold,
        "reentry_pending_count": reentry_pending_count,
        "reentry_confirm_cycles": confirm_cycles,
        "inventory_rebalance_active": inventory_rebalance_active,
        "target_inventory_levels": target_inventory_levels if inventory_rebalance_active else None,
        "switched": switched,
    }


def limit_synthetic_inventory_by_distance(
    *,
    plan: dict[str, Any],
    displacement_side: str,
    current_long_notional: float,
    current_short_notional: float,
    target_inventory_levels: int | None,
    per_order_notional: float,
    step_size: float | None,
    min_qty: float | None,
    min_notional: float | None,
) -> dict[str, Any]:
    safe_side = str(displacement_side or "").strip().lower()
    safe_target_levels = max(int(target_inventory_levels or 0), 0)
    safe_per_order_notional = max(float(per_order_notional), 1e-12)
    report = {
        "enabled": safe_side in {"long", "short"},
        "active": False,
        "displacement_side": safe_side,
        "target_inventory_levels": safe_target_levels,
        "target_inventory_notional": safe_target_levels * safe_per_order_notional,
        "current_inventory_levels": 0,
        "current_inventory_notional": 0.0,
        "missing_inventory_levels": 0,
        "excess_notional": 0.0,
        "trimmed_entry_order_count": 0,
        "trimmed_bootstrap_order_count": 0,
    }
    if safe_side not in {"long", "short"} or safe_target_levels <= 0:
        return report

    current_notional = max(
        float(current_long_notional if safe_side == "long" else current_short_notional),
        0.0,
    )
    current_levels = max(int(math.floor((current_notional / safe_per_order_notional) + 1e-12)), 0)
    target_notional = safe_target_levels * safe_per_order_notional
    missing_levels = max(safe_target_levels - current_levels, 0)
    excess_notional = max(current_notional - target_notional, 0.0)
    report.update(
        {
            "active": True,
            "current_inventory_levels": current_levels,
            "current_inventory_notional": current_notional,
            "missing_inventory_levels": missing_levels,
    ...
    def _is_same_side_entry(order: dict[str, Any]) -> bool:
        role = str(order.get("role", "")).strip().lower()
        side = str(order.get("side", "")).strip().upper()
        if safe_side == "long":
            return side == "BUY" and role in {"entry_long", "bootstrap_long"}
        return side == "SELL" and role in {"entry_short", "bootstrap_short"}

    trimmed_bootstrap = [order for order in bootstrap_orders if _is_same_side_entry(order)]
    bootstrap_orders = [order for order in bootstrap_orders if not _is_same_side_entry(order)]
    report["trimmed_bootstrap_order_count"] = len(trimmed_bootstrap)

    if safe_side == "long":
        entry_orders = [order for order in buy_orders if _is_same_side_entry(order)]
        other_orders = [order for order in buy_orders if not _is_same_side_entry(order)]
        kept_entries = entry_orders[:missing_levels]
        plan["buy_orders"] = [*kept_entries, *other_orders]
        plan["sell_orders"] = sell_orders
    else:
        entry_orders = [order for order in sell_orders if _is_same_side_entry(order)]
        other_orders = [order for order in sell_orders if not _is_same_side_entry(order)]
        kept_entries = entry_orders[:missing_levels]
        plan["sell_orders"] = [*kept_entries, *other_orders]
        plan["buy_orders"] = buy_orders
    plan["bootstrap_orders"] = bootstrap_orders
    report["trimmed_entry_order_count"] = max(len(entry_orders) - len(kept_entries), 0)

    if safe_side == "long":
        plan["buy_orders"] = sorted(
            plan["buy_orders"],
            key=lambda item: (_safe_float(item.get("price")), int(item.get("level", 0) or 0)),
            reverse=True,
        )
    else:
...
def apply_active_delever_long(
    *,
    ...
    synthetic_unmatched_trade_count: int | float | None = None,
    forced_target_notional: float | None = None,
    ...
    guard_reason = _synthetic_active_delever_guard_reason(
        side="long",
        current_qty=current_long_qty,
        current_avg_price=current_long_avg_price,
        unmatched_trade_count=synthetic_unmatched_trade_count,
    )
    if guard_reason:
        report["guard_reason"] = guard_reason
        return report
    threshold_notional = max(_safe_float(threshold_position_notional), 0.0)
    ...
    forced_target = max(_safe_float(forced_target_notional), 0.0)
    if forced_target > 0 and current_long_notional > forced_target + 1e-12:
        safe_per_order_notional = max(float(per_order_notional), 1e-12)
        excess_notional = max(current_long_notional - forced_target, 0.0)
        ...
        trigger_mode = "inventory_rebalance"
    elif threshold_enabled and current_long_notional >= threshold_notional:
...
def apply_active_delever_short(
    *,
    ...
    guard_reason = _synthetic_active_delever_guard_reason(
        ...
        current_avg_price=current_short_avg_price,
        unmatched_trade_count=synthetic_unmatched_trade_count,
    )
    if guard_reason:
        report["guard_reason"] = guard_reason
    ...
    trigger_mode: str | None = None
    forced_target = max(_safe_float(forced_target_notional), 0.0)
    if forced_target > 0 and current_short_notional > forced_target + 1e-12:
        safe_per_order_notional = max(float(per_order_notional), 1e-12)
        excess_notional = max(current_short_notional - forced_target, 0.0)
        active_count = min(len(take_profit_orders), requested_levels, max(1, int(math.ceil(excess_notional / safe_per_order_notional))))
        ...
        trigger_mode = "inventory_rebalance"
    elif threshold_timeout_active and timeout_target > 0 and current_short_notional > timeout_target:
        safe_per_order_notional = max(float(per_order_notional), 1e-12)
...
def _single_synthetic_delever_order(
    *,
    side: str,
    role: str,
    price: float,
    ...
    safe_price = _round_order_price(max(float(price), 0.0), tick_size, normalized_side)
    if safe_price <= 0:
        return None
    qty = _round_order_qty(
        min(max(float(available_qty), 0.0), max(float(max_notional), 0.0) / safe_price),
        step_size,
    )
    if qty <= 0:
        return None
...
def _build_threshold_target_reduce_order(
    *,
    side: str,
    price: float,
    ...
    normalized_side = side.upper().strip()
    safe_price = _round_order_price(max(_safe_float(price), 0.0), tick_size, normalized_side)
    if safe_price <= 0:
        return None
    qty = _round_order_qty(
        min(max(_safe_float(available_qty), 0.0), max(_safe_float(reduce_notional), 0.0) / safe_price),
        step_size,
    )
    if qty <= 0:
        return None
    ...
    return {
        ...
        "role": "forced_reduce",
        "position_side": "BOTH",
        "execution_type": "aggressive" if aggressive else "post_only",
        "time_in_force": "GTC" if aggressive else "GTX",
        "force_reduce_only": True,
    }


def apply_threshold_target_reduce(
    *,
    plan: dict[str, Any],
    state: dict[str, Any],
    now: datetime,
    ...
    min_qty: float | None,
    min_notional: float | None,
    taker_timeout_seconds: float = 60.0,
) -> dict[str, Any]:
    safe_ratio = max(_safe_float(target_ratio), 0.0)
    state_key = "threshold_target_reduce"
    safe_now = now.astimezone(timezone.utc) if now.tzinfo else now.replace(tzinfo=timezone.utc)
    timeout_seconds = max(_safe_float(taker_timeout_seconds), 0.0)
    report: dict[str, Any] = {
        "enabled": safe_ratio > 0,
        "active": False,
        "target_ratio": safe_ratio,
        "taker_timeout_seconds": timeout_seconds,
        "long_active": False,
        "short_active": False,
        "long_elapsed_seconds": 0.0,
        "short_elapsed_seconds": 0.0,
        "long_taker_timeout_active": False,
        "short_taker_timeout_active": False,
        "long_target_notional": None,
        "short_target_notional": None,
        "long_reduce_notional": 0.0,
        "short_reduce_notional": 0.0,
        "dropped_long_entry_orders": 0,
        "dropped_short_entry_orders": 0,
    ...
    if long_pause > 0 and current_long_qty > 0 and current_long_notional > long_pause:
        entered_at = _parse_iso_datetime(timeout_state.get("long_entered_at")) or safe_now
        long_elapsed = max((safe_now - entered_at.astimezone(timezone.utc)).total_seconds(), 0.0)
        long_taker_active = timeout_seconds > 0 and long_elapsed >= timeout_seconds
        timeout_state["long_entered_at"] = _isoformat(entered_at)
        target_notional = long_pause * safe_ratio
        reduce_notional = max(_safe_float(current_long_notional) - target_notional, 0.0)
        reduce_order = _build_threshold_target_reduce_order(
            side="SELL",
            price=bid_price if long_taker_active else ask_price,
            reduce_notional=reduce_notional,
            available_qty=current_long_qty,
            tick_size=tick_size,
            step_size=step_size,
            min_qty=min_qty,
            min_notional=min_notional,
            aggressive=long_taker_active,
        )
        previous_bootstrap = list(plan.get("bootstrap_orders", []))
        previous_buys = list(plan.get("buy_orders", []))
        plan["bootstrap_orders"] = [item for item in previous_bootstrap if not _is_long_entry_order(item)]
        plan["buy_orders"] = [item for item in previous_buys if not _is_long_entry_order(item)]
        if reduce_order is not None:
            plan["sell_orders"] = [reduce_order, *list(plan.get("sell_orders", []))]
            report["placed_reduce_orders"] += 1
        report.update(
            {
                "active": True,
                "long_active": True,
                "long_elapsed_seconds": long_elapsed,
                "long_taker_timeout_active": long_taker_active,
                "long_target_notional": target_notional,
                "long_reduce_notional": reduce_notional,
                "dropped_long_entry_orders": (len(previous_bootstrap) - len(plan["bootstrap_orders"]))
                + (len(previous_buys) - len(plan["buy_orders"])),
            }
        )
    else:
        timeout_state["long_entered_at"] = None

    if short_pause > 0 and current_short_qty > 0 and current_short_notional > short_pause:
        entered_at = _parse_iso_datetime(timeout_state.get("short_entered_at")) or safe_now
        short_elapsed = max((safe_now - entered_at.astimezone(timezone.utc)).total_seconds(), 0.0)
        short_taker_active = timeout_seconds > 0 and short_elapsed >= timeout_seconds
        timeout_state["short_entered_at"] = _isoformat(entered_at)
        target_notional = short_pause * safe_ratio
        reduce_notional = max(_safe_float(current_short_notional) - target_notional, 0.0)
        reduce_order = _build_threshold_target_reduce_order(
            side="BUY",
            price=ask_price if short_taker_active else bid_price,
            reduce_notional=reduce_notional,
            available_qty=current_short_qty,
            tick_size=tick_size,
            step_size=step_size,
            min_qty=min_qty,
            min_notional=min_notional,
            aggressive=short_taker_active,
        )
        previous_bootstrap = list(plan.get("bootstrap_orders", []))
        previous_sells = list(plan.get("sell_orders", []))
        plan["bootstrap_orders"] = [item for item in previous_bootstrap if not _is_short_entry_order(item)]
        plan["sell_orders"] = [item for item in previous_sells if not _is_short_entry_order(item)]
        if reduce_order is not None:
            plan["buy_orders"] = [reduce_order, *list(plan.get("buy_orders", []))]
            report["placed_reduce_orders"] += 1
        report.update(
            {
                "active": True,
                "short_active": True,
                "short_elapsed_seconds": short_elapsed,
                "short_taker_timeout_active": short_taker_active,
                "short_target_notional": target_notional,
                "short_reduce_notional": reduce_notional,
                "dropped_short_entry_orders": (len(previous_bootstrap) - len(plan["bootstrap_orders"]))
                + (len(previous_sells) - len(plan["sell_orders"])),
            }
        )
    else:
        timeout_state["short_entered_at"] = None
...
def apply_synthetic_overweight_deleveraging(
    *,
    ...
    take_profit_min_profit_ratio: float | None,
) -> dict[str, Any]:
    state_key = "synthetic_overweight_delever"
    report: dict[str, Any] = {
        "enabled": True,
        "active": False,
        "direction": None,
    ...
    pause_notional = max(_safe_float(pause_short_position_notional), 0.0)
    max_notional = max(_safe_float(max_short_position_notional), 0.0)
    if current_short_qty <= 0 or pause_notional <= 0 or current_short_notional < pause_notional:
        state.pop(state_key, None)
        return report

    safe_now = now.astimezone(timezone.utc) if now.tzinfo else now.replace(tzinfo=timezone.utc)
    previous = state.get(state_key) if isinstance(state.get(state_key), dict) else {}
    entered_at = _parse_iso_datetime((previous or {}).get("entered_at"))
    previous_qty = _safe_float((previous or {}).get("last_position_qty"))
    if str((previous or {}).get("direction") or "") != "short" or entered_at is None or current_short_qty < previous_qty - 1e-9:
        entered_at = safe_now
    elapsed = max((safe_now - entered_at.astimezone(timezone.utc)).total_seconds(), 0.0)
    hard_overweight = max_notional > 0 and current_short_notional >= max_notional
    phase = "best_quote_timeout" if elapsed >= max(float(timeout_seconds), 0.0) else "profit_only"

    plan["bootstrap_orders"] = [item for item in plan.get("bootstrap_orders", []) if not _is_short_entry_order(item)]
    plan["sell_orders"] = [item for item in plan.get("sell_orders", []) if not _is_short_entry_order(item)]

    if phase == "best_quote_timeout":
        target_price = max(float(bid_price), 0.0)
    else:
        profit_ratio = max(_safe_float(take_profit_min_profit_ratio), 0.0)
        target_price = max(float(current_short_avg_price), 0.0) * (1.0 - profit_ratio)
        if target_price <= 0:
            target_price = min(
                [
                    ...
                    and _order_role(item) in {"take_profit_short", "active_delever_short"}
                    and _safe_float(item.get("price")) > 0
                ]
                or [0.0]
            )

    target_notional = min(
        max(float(per_order_notional), 0.0),
        max(current_short_notional - pause_notional, 0.0) or max(float(per_order_notional), 0.0),
    )
    minimum_working_qty = max(_safe_float(min_qty), _safe_float(step_size), 0.0)
    minimum_working_notional = max(
        _safe_float(min_notional),
        minimum_working_qty * max(float(target_price), 0.0),
    )
    if minimum_working_notional > 0 and 0 < target_notional < minimum_working_notional:
        target_notional = min(
            max(float(per_order_notional), minimum_working_notional),
            current_short_qty * max(float(target_price), 0.0),
        )
    reduce_order = _single_synthetic_delever_order(
        side="BUY",
    ...
    plan["buy_orders"] = [reduce_order] if reduce_order is not None else []
    state[state_key] = {
        "direction": "short",
        "phase": phase,
        "entered_at": _isoformat(entered_at),
        "last_checked_at": _isoformat(safe_now),
        "last_position_qty": current_short_qty,
        "current_notional": current_short_notional,
    ...
    report.update(
        {
            ...
            "reason": (
                f"current_short_notional={_float(current_short_notional)} >= "
                f"pause_short_position_notional={_float(pause_notional)}"
            ),
            "hard_overweight": hard_overweight,
            "target_price": target_price,
            "target_notional": target_notional,
            "rewritten_orders": 1 if reduce_order is not None else 0,
        }
    )
    return report
...
def generate_plan_report(args: argparse.Namespace) -> dict[str, Any]:
    ...
    take_profit_guard = {
        ...
        "effective_min_profit_ratio": max(_safe_float(getattr(args, "take_profit_min_profit_ratio", None)), 0.0),
        "long_active": False,
        "short_active": False,
        "long_cost_basis_missing": False,
        "short_cost_basis_missing": False,
    ...
    threshold_target_reduce = {
        "enabled": _safe_float(getattr(args, "threshold_reduce_target_ratio", 0.0)) > 0,
        "active": False,
        "target_ratio": _safe_float(getattr(args, "threshold_reduce_target_ratio", 0.0)),
        "taker_timeout_seconds": _safe_float(getattr(args, "threshold_reduce_taker_timeout_seconds", 60.0)),
        "long_active": False,
        "short_active": False,
    }
    ...
    synthetic_distance_regime = {
        "enabled": False,
        "zone": "near_market",
        "distance_steps": 0.0,
        "displacement_side": "flat",
        "near_market_active": True,
        "near_market_threshold_steps": getattr(args, "near_market_entry_max_center_distance_steps", 2.0),
        "grid_inventory_rebalance_min_center_distance_steps": getattr(
            args,
            "grid_inventory_rebalance_min_center_distance_steps",
            3.0,
        ...
        "reentry_pending_count": 0,
        "reentry_confirm_cycles": getattr(args, "near_market_reentry_confirm_cycles", 2),
        "inventory_rebalance_active": False,
        "target_inventory_levels": None,
    }
    synthetic_inventory_distance_limit = {
        "enabled": False,
        "active": False,
        "displacement_side": "flat",
        "target_inventory_levels": 0,
        "target_inventory_notional": 0.0,
        "current_inventory_levels": 0,
        "current_inventory_notional": 0.0,
        "missing_inventory_levels": 0,
        "excess_notional": 0.0,
    ...
    if _is_custom_grid_mode(args):
        ...
        plan = build_static_binance_grid_plan(
            ...
            current_long_qty=current_long_qty,
            current_short_qty=current_short_qty,
            bootstrap_positions=custom_direction == "short" and startup_pending,
        )
        ...
        if custom_direction == "neutral":
            ...
            target_base_qty = _safe_float(plan.get("target_base_qty", 0.0))
            bootstrap_qty = _safe_float(plan.get("bootstrap_qty", 0.0))
        else:
        ...
        current_long_qty = max(_safe_float(synthetic_ledger_snapshot.get("virtual_long_qty")), 0.0)
        current_short_qty = max(_safe_float(synthetic_ledger_snapshot.get("virtual_short_qty")), 0.0)
        current_long_avg_price = _resolve_long_cost_basis_price(
            _safe_float(synthetic_ledger_snapshot.get("virtual_long_avg_price")),
            actual_cost_basis_price if actual_net_qty > 1e-12 else 0.0,
        )
        current_short_avg_price = _resolve_short_cost_basis_price(
            _safe_float(synthetic_ledger_snapshot.get("virtual_short_avg_price")),
            actual_cost_basis_price if actual_net_qty < -1e-12 else 0.0,
        )
        current_long_notional = current_long_qty * max(mid_price, 0.0)
        current_short_notional = current_short_qty * max(mid_price, 0.0)
        long_active_delever_guard_reason = _synthetic_active_delever_guard_reason(
            side="long",
            current_qty=current_long_qty,
            current_avg_price=current_long_avg_price,
            unmatched_trade_count=synthetic_ledger_snapshot.get("unmatched_trade_count"),
        )
        short_active_delever_guard_reason = _synthetic_active_delever_guard_reason(
            side="short",
            current_qty=current_short_qty,
            current_avg_price=current_short_avg_price,
            unmatched_trade_count=synthetic_ledger_snapshot.get("unmatched_trade_count"),
        )
        if long_active_delever_guard_reason or short_active_delever_guard_reason:
            state["synthetic_ledger_resync_required"] = True
        inventory_short_pause_active = (
            _safe_float(getattr(effective_args, "pause_short_position_notional", None)) > 0
            and current_short_notional >= _safe_float(getattr(effective_args, "pause_short_position_notional", None)) - 1e-12
        ...
        synthetic_distance_regime = resolve_synthetic_distance_regime(
            state=state,
            center_price=center_price,
            mid_price=mid_price,
            step_price=effective_args.step_price,
            near_market_max_center_distance_steps=getattr(
                effective_args,
                "near_market_entry_max_center_distance_steps",
                2.0,
            ),
            ...
            near_market_reentry_confirm_cycles=getattr(
                effective_args,
        ...
        hedge_plan = build_hedge_micro_grid_plan(
            ...
            near_market_entry_max_center_distance_steps=getattr(
                ...
                "near_market_entry_max_center_distance_steps",
                2.0,
            ) if bool(synthetic_distance_regime.get("near_market_active")) else 0.0,
            buy_offset_steps=(
                float(getattr(effective_args, "static_buy_offset_steps", 0.0))
                + float((market_bias or {}).get("buy_offset_steps", 0.0))
            ),
        ...
        if bool(synthetic_distance_regime.get("inventory_rebalance_active")):
            synthetic_inventory_distance_limit = limit_synthetic_inventory_by_distance(
                plan=plan,
                displacement_side=str(synthetic_distance_regime.get("displacement_side") or "flat"),
                current_long_notional=controls["current_long_notional"],
                current_short_notional=controls["current_short_notional"],
                target_inventory_levels=synthetic_distance_regime.get("target_inventory_levels"),
                per_order_notional=inventory_tier["effective_per_order_notional"],
                step_size=symbol_info.get("step_size"),
                min_qty=symbol_info.get("min_qty"),
                min_notional=symbol_info.get("min_notional"),
        ...
        long_active_delever = apply_active_delever_long(
            ...
            synthetic_unmatched_trade_count=synthetic_ledger_snapshot.get("unmatched_trade_count"),
            forced_target_notional=(
                synthetic_inventory_distance_limit.get("target_inventory_notional")
                if bool(synthetic_inventory_distance_limit.get("active"))
                and str(synthetic_inventory_distance_limit.get("displacement_side")) == "long"
                and _safe_float(synthetic_inventory_distance_limit.get("excess_notional")) > 0
                else None
            ),
        )
        short_active_delever = apply_active_delever_short(
            ...
            forced_target_notional=(
                synthetic_inventory_distance_limit.get("target_inventory_notional")
                if bool(synthetic_inventory_distance_limit.get("active"))
                and str(synthetic_inventory_distance_limit.get("displacement_side")) == "short"
                and _safe_float(synthetic_inventory_distance_limit.get("excess_notional")) > 0
                else None
            ),
        ...
        plan = _generate_competition_inventory_grid_plan(
            ...
            buy_levels=int(getattr(args, "buy_levels", 1) or 1),
            sell_levels=int(getattr(args, "sell_levels", 1) or 1),
    ...
    if _is_synthetic_neutral_mode(strategy_mode) and synthetic_ledger_snapshot is not None:
        synthetic_overweight_delever = apply_synthetic_overweight_deleveraging(
            plan=plan,
            state=state,
            now=plan_now,
            current_short_qty=current_short_qty,
            ...
            max_short_position_notional=effective_args.max_short_position_notional,
            per_order_notional=inventory_tier["effective_per_order_notional"],
            timeout_seconds=float(getattr(effective_args, "short_threshold_timeout_seconds", 60.0) or 60.0),
            take_profit_min_profit_ratio=getattr(effective_args, "take_profit_min_profit_ratio", None),
        )
    else:
        state.pop("synthetic_overweight_delever", None)

    threshold_target_reduce = apply_threshold_target_reduce(
        plan=plan,
        state=state,
        now=plan_now,
        current_long_qty=current_long_qty,
        ...
        pause_long_position_notional=effective_args.pause_buy_position_notional,
        pause_short_position_notional=effective_args.pause_short_position_notional,
        target_ratio=getattr(effective_args, "threshold_reduce_target_ratio", 0.0),
        bid_price=bid_price,
        ask_price=ask_price,
        tick_size=symbol_info.get("tick_size"),
        step_size=symbol_info.get("step_size"),
    ...
    report = {
        ...
        "synthetic_distance_regime": synthetic_distance_regime,
        "synthetic_inventory_distance_limit": synthetic_inventory_distance_limit,
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--market-volume-follow-enabled", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--market-volume-follow-target-divisor", type=float, default=None)
    parser.add_argument("--market-volume-follow-min-scale", type=float, default=0.25)
    parser.add_argument("--market-volume-follow-max-scale", type=float, default=4.0)
    parser.add_argument("--market-volume-follow-refresh-seconds", type=float, default=60.0)
...
def _print_cycle_summary(summary: dict[str, Any]) -> None:
    ...
    if summary.get("strategy_mode") == "synthetic_neutral":
        ...
        if summary.get("synthetic_overweight_delever_active"):
            print(
                "  synthetic_delever: "
                f"{summary.get('synthetic_overweight_delever_phase', '--')} "
                f"elapsed={_float(summary.get('synthetic_overweight_delever_elapsed_seconds', 0.0))}s"
            )
...
def main() -> None:
    ...
    if args.grid_inventory_rebalance_min_center_distance_steps < args.near_market_entry_max_center_distance_steps:
        raise SystemExit("--grid-inventory-rebalance-min-center-distance-steps must be >= --near-market-entry-max-center-distance-steps")
    ...
    if args.threshold_reduce_target_ratio < 0 or args.threshold_reduce_target_ratio >= 1:
    ...
    while True:
        ...
        try:
            ...
            summary = {
                ...
                "synthetic_overweight_delever_active": bool(
                    (plan_report.get("synthetic_overweight_delever") or {}).get("active")
                ),
                "synthetic_overweight_delever_phase": str(
                    ((plan_report.get("synthetic_overweight_delever") or {}).get("phase", "") or "")
                ),
                "synthetic_overweight_delever_reason": (
                    (plan_report.get("synthetic_overweight_delever") or {}).get("reason")
                ),
