...
RUNNER_STRATEGY_PRESETS: dict[str, dict[str, Any]] = {
    ...
    "bard_12h_push_neutral_v2": {
        "label": "BARD 双向冲量 v2",
        "description": "BARDUSDT 专用旧版双向冲量模板。固定细步长、双边 8 档、零底仓，适合高成交窗口短时冲量；后续跨币复用建议改用 BARD 风格双向冲量模板 v1。",
        "startable": True,
        "kind": "synthetic",
        "symbol": "BARDUSDT",
        "config": {
...
<body>
  ...
  <script>
    ...
    const RUNNER_PROFILE_GUIDE_NOTES = {
      ...
      bard_12h_push_neutral_v2: {
        summary: "BARDUSDT 的旧版双向冲量 v2。固定细步长、双边 8 档、零底仓，目标是把高成交窗口里的双边回转密度堆高。",
        focus: [
          "这套关闭了 autotune 和 reduce_only 护栏，适合短时冲量，不适合长期常开；后续跨币复用建议优先用 BARD 风格双向冲量模板 v1。",
        ],
      },
      synthetic_neutral_bard_style_v1: {
