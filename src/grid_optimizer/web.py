...
RUNNER_STRATEGY_PRESETS: dict[str, dict[str, Any]] = {
    ...
    "bard_12h_push_neutral_v2": {
        "label": "通用刷量V1",
        "description": "当前 111/150 在跑的通用刷量模板。步长放宽到 0.0005，并把停买/硬上限收紧到 420U / 650U；轻仓回补默认至少保 1 tick，不再亏损贴边平仓。",
        "startable": True,
        "kind": "synthetic",
        "symbol": "BARDUSDT",
        "config": {
            ...
            "max_new_orders": 40,
            "max_cumulative_notional": 660000.0,
    ...
    "xaut_competition_push_neutral_v1": {
        "label": "XAUT 交易赛刷量 v1",
        "description": "基于通用刷量V1改成的 XAUTUSDT 合约交易赛版。固定 0.1 步长、8 买 4 卖、零底仓，前两档保队列，并在残余空仓很小时直接恢复双边挂单，关闭 autotune 以原样执行。",
        "startable": True,
        "kind": "synthetic",
        "symbol": "XAUTUSDT",
        "config": {
    ...
    "synthetic_neutral_bard_style_v1": {
        "label": "BARD 贴边成交模板 v1",
        "description": "把 114/150 上验证过的 BARD 近价成交骨架整理成通用 synthetic_neutral 模板。固定 0.0007 步长、8 买 4 卖、零底仓，保留贴盘口首档回补的成交风格，适合在量优先场景直接复用。",
        "startable": True,
        "kind": "synthetic",
        "config": {
            "strategy_mode": "synthetic_neutral",
    ...
    "based_overnight_volume_v1": {
        "label": "BASED 隔夜刷量 v1",
        "description": "BASEDUSDT 专用隔夜刷量版。沿用 guarded v2 的 synthetic_neutral 骨架，但把步长收窄到 0.0001、单笔提到 50U，并进一步压低短侧库存与累计成交额停机线，适合夜间连续冲量后在累计 25 万或运行窗口结束前退出。",
        "startable": True,
        "kind": "synthetic",
        "symbol": "BASEDUSDT",
        "config": {
            ...
            "base_position_notional": 0.0,
            "sticky_entry_levels": 1,
            "synthetic_residual_long_flat_notional": 400.0,
            "synthetic_residual_short_flat_notional": 300.0,
            "flat_start_enabled": True,
            "warm_start_enabled": True,
            "up_trigger_steps": 1,
            "down_trigger_steps": 1,
            ...
            "max_cumulative_notional": 250000.0,
            "excess_inventory_reduce_only_enabled": False,
            "autotune_symbol_enabled": False,
            "volume_trigger_enabled": False,
            "volume_trigger_window": "15m",
...
def _volatility_trigger_orphan_recovery_action(
    config: dict[str, Any],
    previous_status: dict[str, Any],
    *,
    signal_hit: bool,
    ...
    if not signal_hit or runner_running or flatten_running:
        return None
    if not _volatility_trigger_status_from_stop(previous_status):
        return None
    phase = str(previous_status.get("phase") or "").strip()
    if phase == "reduce_to_notional":
        if bool(previous_status.get("reduce_target_reached", False)):
            return None
        if reduce_target_notional > 0:
            return {"action": "reduce_to_notional", "reason": "retry_reduce_to_notional"}
    if phase == "full_flatten":
        if bool(previous_status.get("full_flatten_target_reached", False)):
            return None
        if bool(config.get("volatility_trigger_stop_close_all_positions", True)):
            return {"action": "full_flatten", "reason": "retry_full_flatten"}
    if phase not in {"reduce_to_notional", "full_flatten"} and reduce_target_notional > 0:
        return {"action": "reduce_to_notional", "reason": "volatility_above_threshold"}
    return None
...
def _reconcile_runner_volatility_trigger(config: dict[str, Any]) -> None:
    ...
    if (
        ...
        recovery_action = _volatility_trigger_orphan_recovery_action(
            normalized_config,
            previous_status,
        ...
        if recovery_action is not None:
            decision["action"] = recovery_action["action"]
            decision["reason"] = recovery_action["reason"]
    if (
        signal_hit
    ...
    try:
        if decision.get("action") == "start":
            ...
            status["reduce_max_loss_ratio"] = None
            ...
            if decision.get("action") == "reduce_to_notional":
                ...
                if reduce_max_loss_ratio is not None:
                    ...
                    close_position_max_loss_ratio = reduce_max_loss_ratio
            ...
            if target_notional > 0:
                ...
                warnings = post_actions.get("warnings") if isinstance(post_actions.get("warnings"), list) else []
...
def _build_runner_command(config: dict[str, Any]) -> list[str]:
    ...
    if config.get("synthetic_residual_long_flat_notional") is not None:
        command.extend(
            [
                "--synthetic-residual-long-flat-notional",
                str(config["synthetic_residual_long_flat_notional"]),
            ]
        )
    if config.get("synthetic_residual_short_flat_notional") is not None:
        command.extend(
            [
                "--synthetic-residual-short-flat-notional",
                str(config["synthetic_residual_short_flat_notional"]),
            ]
        )
    if config.get("synthetic_tiny_long_residual_notional") is not None:
        command.extend(
            [
                "--synthetic-tiny-long-residual-notional",
                str(config["synthetic_tiny_long_residual_notional"]),
            ]
        )
    if config.get("synthetic_tiny_short_residual_notional") is not None:
        command.extend(
            [
...
def _build_flatten_command(config: dict[str, Any]) -> list[str]:
    ...
    max_loss_ratio = float(config.get("max_loss_ratio") or 0.0)
    if max_loss_ratio > 0:
        command.extend(["--max-loss-ratio", str(max_loss_ratio)])
    return command
...
def _start_flatten_process(config: dict[str, Any]) -> dict[str, Any]:
    ...
    desired = {
        ...
        "max_loss_ratio": max(float(config.get("max_loss_ratio") or 0.0), 0.0),
    }
...
def _build_stop_execution_summary(
    *,
    ...
    max_loss_ratio = max(float(close_position_max_loss_ratio or 0.0), 0.0)
    return {
        "symbol": symbol,
        ...
        "close_position_allow_loss": bool(close_position_allow_loss),
        "close_position_max_loss_ratio": max_loss_ratio,
    ...
    *,
    ...
    if close_all_positions:
        ...
        live_snapshot = load_live_flatten_snapshot(
            ...
            allow_loss=bool(close_position_allow_loss),
...
<body>
  <div class="wrap">
    ...
    <section class="card">
      ...
      <div class="toolbar runtime-guard-toolbar">
        ...
        <label>减仓最大亏损比例
          <input id="monitor_volatility_trigger_reduce_max_loss_ratio" type="number" min="0" step="0.0001" />
        </label>
  ...
  <script>
    ...
    const monitorVolatilityTriggerReduceMaxLossRatioEl = document.getElementById("monitor_volatility_trigger_reduce_max_loss_ratio");
    ...
    const LOCAL_STRATEGY_PRESETS = [
      ...
      {
        key: "xaut_competition_push_neutral_v1",
        label: "XAUT 交易赛刷量 v1",
        description: "基于通用刷量V1改成的 XAUTUSDT 合约交易赛版。固定 0.1 步长、8 买 4 卖、零底仓，前两档保队列，并在残余空仓很小时直接恢复双边挂单，关闭 autotune 以原样执行。",
        startable: true,
        kind: "synthetic",
      ...
      {
        key: "synthetic_neutral_bard_style_v1",
        label: "BARD 贴边成交模板 v1",
        description: "把 114/150 上验证过的 BARD 近价成交骨架整理成通用 synthetic_neutral 模板。固定 0.0007 步长、8 买 4 卖、零底仓，保留贴盘口首档回补的成交风格，适合在量优先场景直接复用。",
        startable: true,
        kind: "synthetic",
      ...
      },
      {
        ...
        config: {
          ...
          take_profit_min_profit_ratio: 0,
          market_bias_enabled: true,
          market_bias_max_shift_steps: 0.75,
          market_bias_signal_steps: 2,
          market_bias_drift_weight: 0.65,
    ...
    const RUNNER_PARAM_EXPLAIN = {
      ...
      volatility_trigger_reduce_max_loss_ratio: "分段减仓允许的最大亏损比例。留空时沿用默认保本保护；设置后只在该亏损比例以内跟价减仓。",
    ...
    function readRuntimeGuardConfigFromInputs() {
      return {
        ...
        volatility_trigger_stop_reduce_to_notional: monitorVolatilityTriggerStopReduceToNotionalEl && monitorVolatilityTriggerStopReduceToNotionalEl.value
          ...
          : null,
        volatility_trigger_reduce_max_loss_ratio: monitorVolatilityTriggerReduceMaxLossRatioEl && monitorVolatilityTriggerReduceMaxLossRatioEl.value
          ? Number(monitorVolatilityTriggerReduceMaxLossRatioEl.value)
          : null,
    ...
    function syncRuntimeGuardInputsFromConfig(config) {
      ...
      monitorVolatilityTriggerReduceMaxLossRatioEl.value = source.volatility_trigger_reduce_max_loss_ratio ?? "";
    ...
    const RUNNER_PROFILE_GUIDE_NOTES = {
      ...
      bard_12h_push_neutral_v2: {
        summary: "当前 111/150 在跑的通用刷量V1。现在改成 0.0005 步长、8 买 4 卖、零底仓，并给轻仓回补补上至少 1 tick 的利润底线。",
        focus: [
          "这套就是当前线上通用刷量基线，目标是在保住成交密度的前提下，减少趋势段里旧仓贴着盘口亏损回补。",
          "标签改名后，后续部署和观察都统一按 通用刷量V1 识别，不再和旧的 BARD 双向冲量 v2 混用。",
        ],
      },
      xaut_competition_push_neutral_v1: {
        summary: "给 XAUTUSDT 的合约交易赛刷量版。直接继承通用刷量V1的 8 买 4 卖和零底仓结构，但把步长固定成 0.1。",
        focus: [
          "这套刻意关闭 autotune_symbol_enabled，目的就是避免 XAUT 高价位在启动前把 step_price 自动抬大，最终偏离你想要的 0.1 节奏。",
          "另外把 sticky_entry_levels 设成 2，前两档贴盘口单会优先沿用更接近盘口的旧价位，减少因为中心或盘口小幅漂移带来的近端重挂。",
          "新增 synthetic_residual_short_flat_notional=30，残余空仓低于约 30U 时会直接按平仓处理，不再因为一点点 short 残量缩成单边回补。",
          "其余仓位护栏和累计成交额停机线沿用通用刷量V1，适合作为 XAUT 交易赛的固定节奏基线，再按实际盘口密度继续细调单笔名义和库存上限。",
        ],
      },
      synthetic_neutral_bard_style_v1: {
        summary: "把 114/150 上跑稳的 BARD 近价成交骨架抽成通用模板。固定 0.0007 步长、8 买 4 卖、零底仓，保留贴盘口首档回补的成交风格。",
        focus: [
          "这套就是保留下来的贴边成交版模板，后续如果更看重吃到盘口流动性而不是单笔利润，可以直接拿它开跑。",
          "它不是跨所有币种即插即用的万能参数集；迁移时至少要重审 step_price、per_order_notional 和多空仓位上限，确认和该币的 tick、盘口密度、可承受库存匹配。",
          "默认关闭 autotune_symbol_enabled 和 volatility_trigger_enabled，是为了先保留实盘验证过的固定节奏；如果换到更高价或更稀疏盘口的币，优先改步长，再决定要不要开自适应。",
        ],
      },
      based_volume_long_trigger_v1: {
      ...
      based_volume_guarded_bard_v2: {
        summary: "给 BASED 定制的固定节奏双向冲量版。保留 BARD 风格骨架，但把步长固定在 0.0002，收紧空侧和总仓位，再叠加轻仓保本回补、量能启停与小时亏损熔断。",
        focus: [
          "这套刻意关闭 adaptive step，目的是避免在 BASED 的急波动里被自动拉宽报价后突然失去成交；核心靠固定步长和更低的仓位阈值控损。",
          "空仓还轻的时候，BUY 回补单会参考 synthetic ledger 的空仓均价，默认先保本再出手；只有空仓名义逼近软停阈值后，才放宽成原始回补价。",
          "最近 15 分钟成交额掉到阈值以下时会自动撤单并清仓，更适合高成交窗口短开，不适合全天无差别常开。",
        ],
      },
      based_overnight_volume_v1: {
        summary: "给 BASED 做的隔夜刷量版。沿用 guarded v2 的控损骨架，但收窄到 0.0001 步长、50U 单笔，并用更低的短侧阈值换更持续的轻仓刷量。",
        focus: [
          "这套默认关掉量能和高波动门禁，避免夜里因为市场成交回落或波动短时放大而直接停机；主要靠 max_cumulative_notional=250000 和运行窗口控制结束。",
          "通过 sticky_entry_levels=1、synthetic_residual_long_flat_notional=400 和 synthetic_residual_short_flat_notional=300，让轻仓多空两侧都更早恢复双边贴盘口，优先保成交量。",
          "它不是全天常驻模板，适合几小时的夜间短开；想刷得更凶要继续放松短侧阈值，但损耗和单边库存都会明显抬升。",
        ],
      },
      based_volume_push_bard_v1: {
        summary: "通用的双向冲量自适应模板。启动先按当前币种的价位、tick 和点差自动估一个基础步长与单笔名义，再用 adaptive step 在急波动和持续单边时把继续接仓速度压下去。",
        focus: [
          "这套更适合成交明显抬升时段的 burst 型短开；它不是固定参数集。想复用 114/150 那种固定节奏，应该改用 BARD 风格双向冲量模板 v1。",
        ],
      },
      xaut_long_adaptive_v1: {
    ...
    function buildExecutionGuardLines(config) {
      ...
      if (config.volatility_trigger_enabled) {
        ...
        if (reduceTarget > 0) {
          ...
          if (asGuideNumber(config.volatility_trigger_reduce_max_loss_ratio) > 0) {
            stopActions.push(`减仓限损 ${fmtGuidePctFromRatio(config.volatility_trigger_reduce_max_loss_ratio)}`);
          }
    ...
    [
      ...
      monitorVolatilityTriggerStopReduceToNotionalEl,
      monitorVolatilityTriggerReduceMaxLossRatioEl,
...
def _run_loop_monitor_query(query: dict[str, list[str]]) -> dict[str, Any]:
    ...
    snapshot["volatility_trigger"] = {
        ...
        "reduce_max_loss_ratio": runner_config.get("volatility_trigger_reduce_max_loss_ratio"),
