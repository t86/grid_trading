...
RUNNER_STRATEGY_PRESETS: dict[str, dict[str, Any]] = {
    ...
    "xaut_volume_guarded_bard_v2": {
        "label": "XAUT 冲量控损 v2",
        "description": "仅用于 XAUTUSDT 的固定节奏 synthetic_neutral。沿用 111 线上实盘版本：逐笔 lot 一格配对止盈、8 买 5 卖、单笔 20U、对称 2000/2500 仓位阈值，并固定 10x 杠杆。",
        "startable": True,
        "kind": "synthetic",
        "symbol": "XAUTUSDT",
        "config": {
...
<body>
  ...
  <script>
    ...
    const RUNNER_PROFILE_GUIDE_NOTES = {
      ...
      xaut_volume_guarded_bard_v2: {
        summary: "XAUTUSDT 的固定节奏双向冲量版。沿用 111 线上实盘版本：8 买 5 卖、单笔 20U、10x 杠杆，并把多空软硬阈值对称放到 2000 / 2500。",
        focus: [
