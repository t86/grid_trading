...
RUNNER_STRATEGY_PRESETS: dict[str, dict[str, Any]] = {
    ...
    "synthetic_neutral_bard_style_v1": {
        "label": "BARD 风格双向冲量模板 v1",
        "description": "把 114/150 上验证过的 BARD 实盘骨架整理成通用 synthetic_neutral 模板。固定 0.0007 步长、8 买 4 卖、零底仓，默认关闭 autotune 和波动停机，适合复制给同价位币种后再按盘口与风险上限细调。",
        "startable": True,
        "kind": "synthetic",
        "config": {
            "strategy_mode": "synthetic_neutral",
...
<body>
  ...
  <script>
    ...
    const LOCAL_STRATEGY_PRESETS = [
      ...
      },
      {
        ...
        config: {
          ...
          volume_trigger_enabled: true,
          volume_trigger_window: "15m",
          volume_trigger_start_threshold: 1000000.0,
          volume_trigger_stop_threshold: 700000.0,
          volume_trigger_stop_cancel_open_orders: true,
          volume_trigger_stop_close_all_positions: false,
        },
      },
    ...
    const RUNNER_PROFILE_GUIDE_NOTES = {
      ...
      synthetic_neutral_bard_style_v1: {
        summary: "把 114/150 上跑稳的 BARD 骨架抽成通用模板。固定 0.0007 步长、8 买 4 卖、零底仓，先保留验证过的节奏和不对称仓位护栏。",
        focus: [
