...
class RuntimeGuardConfig:
    ...
    max_cumulative_loss_limit: float | None = None
...
def normalize_runtime_guard_config(raw: dict[str, Any]) -> RuntimeGuardConfig:
    config = RuntimeGuardConfig(
        ...
        max_cumulative_loss_limit=_parse_positive_float(
            raw.get("max_cumulative_loss_limit"),
...
def normalize_runtime_guard_payload(
    ...
    *,
    ...
    return {
        ...
        "max_cumulative_loss_limit": config.max_cumulative_loss_limit,
    ...
    *,
    ...
    for event in pnl_events:
        ...
        if config.runtime_guard_stats_start_time and event_ts < config.runtime_guard_stats_start_time:
            continue
        event_net_pnl = _event_net_pnl(event)
        cumulative_net_pnl += event_net_pnl
        if event_ts < window_start:
            continue
        window_net_pnl += event_net_pnl
    rolling_loss = max(0.0, -window_net_pnl)
    cumulative_loss = max(0.0, -cumulative_net_pnl)
    ...
    if config.max_cumulative_loss_limit is not None and cumulative_loss >= config.max_cumulative_loss_limit:
        reasons.append("max_cumulative_loss_limit_hit")
