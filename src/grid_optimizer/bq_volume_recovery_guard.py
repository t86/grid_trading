...
def recover_arx_effective_control_drift(
    *,
    ...
    if control_sticky is not False:
        ...
        # Until then, retain no acknowledgement from an earlier false value.
        item.pop("effective_control_drift_sticky_value", None)
        return None
    if plan.get("effective_sticky_entry_preserve_less_aggressive") is False:
        return None
    if item.get("effective_control_drift_sticky_value") is False:
        return {
            "symbol": normalized_symbol,
            "action": "hold_arx_effective_control_drift_already_restarted",
            "changed_keys": [],
            "backup_path": None,
            "dry_run": dry_run,
            "restart_failed": None,
    ...
    item.update(
        {
            "last_effective_control_drift_restart_at": now.isoformat(),
            "effective_control_drift_sticky_value": False,
            "last_recovery_action_at": now.isoformat(),
...
def check_symbol(
    *,
    ...
    volatility_pause = plan.get("volatility_entry_pause")
    volatility_pause = (
        volatility_pause if isinstance(volatility_pause, dict) else {}
    )
    directional_recovery = volatility_pause.get("directional_recovery")
    directional_recovery = (
        directional_recovery if isinstance(directional_recovery, dict) else {}
    )
    inventory_unlock_release = plan.get("inventory_unlock_release")
    inventory_unlock_release = (
        inventory_unlock_release
        if isinstance(inventory_unlock_release, dict)
        else {}
    )
    grvt_post_shock_soft_release_safe = (
        ...
        and _safe_float(directional_recovery.get("seconds_since_trigger"))
        >= max(
            _safe_float(directional_recovery.get("min_observation_seconds")),
            180.0,
        )
    ...
    try:
        if (
            ...
            # wrapper rebuilds it; this guard never submits an order itself.
            action = (
            ...
            * 0.25
            ...
            if arx_balanced_capacity_updates:
                ...
                if current_gap >= configured_gap and relief_gap > configured_gap:
                    ...
                    # A bias boundary is not a valid terminal state when the
                    # exchange still lacks an ordinary entry leg. Restarting
                    # the managed runner is the bounded recovery path; it
                    # lets the wrapper rebuild the missing side without the
                    # guard ever placing or cancelling an order itself.
                    missing_entry_leg = (
                        _safe_int(
                            assessment.get("ordinary_active_entry_long_order_count")
                        ) <= 0
    ...
    for symbol in _normalize_symbols(args.symbols):
        ...
        if effective_control_drift_result is not None:
            ...
            if effective_control_drift_result.get("restart_failed"):
                exit_code = 1
            if not effective_control_drift_result.get("allow_recovery_check"):
                continue
