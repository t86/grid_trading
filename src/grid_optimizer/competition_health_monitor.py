...
def main() -> None:
    ...
    intended = any(x in jout for x in ("stop_reason", "cooling_down", "after_end_window",
                                       "max_cumulative", "runtime_guard_loss_recovery",
                                       "net_loss_reduce", "pause:", "buy_paused=yes",
    ...
    try:
        if not did_restart and active and looping:
            ...
            terminal = any(x in dj for x in ("stop_reason", "cooling_down", "after_end_window",
                                             "max_cumulative", "runtime_guard_loss_recovery"))
    ...
    try:
        ...
        # Gate on `active`: the brake/release apply a config change with a restart, which must
        # never re-start a runner the gate has intentionally stopped at its target.
        if not did_restart and active and vol >= a.first and day_wear < a.hard_wear:
