...
def _bootstrap_orders(
    *,
    ...
    buy_levels: int,
    sell_levels: int,
    max_order_position_notional: float | None,
    tick_size: float | None,
    ...
    if market_type == "futures":
        buy_cap = max_order_position_notional if max(_safe_float(max_order_position_notional), 0.0) > 0 else None
        orders = _build_buy_ladder_orders(
            ...
            next_role="bootstrap_entry",
            start_level=0,
            max_total_notional=buy_cap,
            tick_size=tick_size,
            step_size=step_size,
            min_qty=min_qty,
...
def build_inventory_grid_orders(
    *,
    ...
    if direction_state == "long_active":
        ...
        closeable_qty = _round_order_qty(max(held_qty, 0.0), step_size)
        planned_closing_qty = 0.0
        raw_buy_orders = _build_buy_ladder_orders(
            reference_price=anchor_price,
            step_price=step_price,
            levels=buy_levels if risk_state == "normal" else 1,
            first_order_notional=per_order_notional,
            per_order_notional=per_order_notional,
            first_role="grid_exit",
            next_role="grid_exit",
            start_level=1,
            max_total_notional=closeable_qty * max(exit_price := _round_order_price(anchor_price - step_price, tick_size, "BUY"), 0.0)
            if closeable_qty > 0
            else 0.0,
            tick_size=tick_size,
            step_size=step_size,
        ...
        for order in raw_buy_orders:
            remaining_qty = _round_order_qty(max(closeable_qty - planned_closing_qty, 0.0), step_size)
            if remaining_qty <= 0:
                break
            qty = min(max(_safe_float(order.get("qty")), 0.0), remaining_qty)
            price = max(_safe_float(order.get("price")), 0.0)
            if not _order_meets_mins(qty=qty, price=price, min_qty=min_qty, min_notional=min_notional):
                continue
            buy_orders.append(_build_order(side="BUY", price=price, qty=qty, role="grid_exit"))
            planned_closing_qty += qty
        ...
        if entry_allowed:
            ...
            sell_orders.extend(
                _build_sell_ladder_orders(
                    ...
                    max_total_qty=(
                        _round_order_qty(
                            max(remaining_entry_notional_cap, 0.0) / max(_round_order_price(anchor_price + step_price, tick_size, "SELL"), EPSILON),
                            step_size,
                        )
                        if remaining_entry_notional_cap is not None
                        else None
                    ),
                    tick_size=tick_size,
                    step_size=step_size,
